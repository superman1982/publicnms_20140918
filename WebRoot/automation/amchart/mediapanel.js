...
Ext.apply(Ext.ux.MediaBase.prototype , {
     ...
     mediaMarkup : function(mediaCfg){
         ...
         if( m.url && media.type ){
             ...
             //设置chart_id 属性 			by chenqing 2010-9-25
             m.id || (m.id = Ext.id());
             if(params && params.flashVars) params.flashVars.chart_id = m.id;
             // end ;)
...
Ext.ux.MediaPanel.Flash = Ext.extend(Ext.ux.MediaPanel, {
	...
	//刷新
 	,refresh : function(){
 		this.renderMedia(this.initialConfig.mediaCfg)
 	}
     //Private
