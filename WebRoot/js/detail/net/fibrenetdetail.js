...
NetDetail.prototype = {
	...
	show: function (){
		...
		if(freshSystemInfo){
			freshSystemInfo.onclick = function (){
				FibreNetRemoteService.getHostNodeInfo(ObjectSelf.nodeid, ObjectSelf.type, ObjectSelf.subtype, {
		...
		FibreNetRemoteService.getHostNodeInfo(this.nodeid, this.type, this.subtype, {
			callback:function(data){
				ObjectSelf.hostNode =  data;
				ObjectSelf.getSystemInfo();
			}
	...
	getSystemInfo: function (){
		FibreNetRemoteService.getSystemInfo(this.nodeid, this.type, this.subtype, {
			callback:function(data){
	...
	createSystemInfo: function (){ 
		...
		FibreNetRemoteService.getCurrDayPingAvgInfo(this.nodeid, this.type, this.subtype, {
			callback:function(data){
		...
		FibreNetRemoteService.getCurrCpuAvgInfo(this.nodeid, this.type, this.subtype, {
			callback:function(data){
		...
		FibreNetRemoteService.getStautsInfo(this.nodeid, this.type, this.subtype, {
			callback:function(data){
				var statusImg = "";
				if(data){
					statusImg = data;
		...
		FibreNetRemoteService.getCategoryInfo(this.nodeid, this.type, this.subtype, {
			callback:function(data){
				var category = "";
				if(data){
					category = data;
		...
		FibreNetRemoteService.getSupperInfo(this.nodeid, this.type, this.subtype, {
			callback:function(data){
				var supperInfo = "";
				if(data){
					supperInfo = data;
	...
	// 获取详细信息页面上标签页的信息
	getTabInfo: function (){
		FibreNetRemoteService.getTabInfo(this.nodeid, this.type, this.subtype, {
			callback:function(data){
	...
	getInterfaceInfo: function(){
		FibreNetRemoteService.getInterfaceInfo(this.nodeid, this.type, this.subtype, {
			callback:function(data){
	...
	// 获取设备 ARP 信息
	getARPInfo: function (){
		FibreNetRemoteService.getARPInfo(this.nodeid, this.type, this.subtype, {
			callback:function(data){
	...
	// 获取设备 FDB 信息
	getFDBInfo: function (){
		FibreNetRemoteService.getFDBInfo(this.nodeid, this.type, this.subtype, {
			callback:function(data){
	...
	getRouterInfo: function (){
		FibreNetRemoteService.getRouterInfo(this.nodeid, this.type, this.subtype, {
			callback:function(data){
	...
	getProcessInfo: function(){
		FibreNetRemoteService.getProcessInfo(this.nodeid, this.type, this.subtype, {
			callback:function(data){
	...
	createProcessInfo: function(){
		...
		var currTabId = this.currTab.id;
		var pid_cellIndex = $(currTabId + "-titleTD-" + "pid").cellIndex;
		var name_cellIndex = $(currTabId + "-titleTD-" + "name").cellIndex;
		var averageUSecs_cellIndex = $(currTabId + "-titleTD-" + "averageUSecs").cellIndex;
		var extPriorityRev_cellIndex = $(currTabId + "-titleTD-" + "extPriorityRev").cellIndex;
		var runtime_cellIndex = $(currTabId + "-titleTD-" + "runtime").cellIndex;
		...
		while(i < length){
			...
			var averageUSecs = processInfo.averageUSecs;		// 平均消耗CPU运行时间(光纤交换机)
			var extPriorityRev = processInfo.extPriorityRev;	// 优先级(光纤交换机)
			var runtime = processInfo.runtime;					// 运行时间(光纤交换机)
			...
			// 进程ID
			var td = detailDataTDArray[pid_cellIndex];
			td.id = this.currTab.id + "-dataTD-" + index + "-" + "pid";
			...
			// 平均消耗CPU运行时间(光纤交换机)
			var td = detailDataTDArray[averageUSecs_cellIndex];
			td.id = this.currTab.id + "-dataTD-" + index + "-" + "averageUSecs";
			td.insertAdjacentHTML("afterBegin", averageUSecs);
			
			// 优先级(光纤交换机)
			var td = detailDataTDArray[extPriorityRev_cellIndex];
			td.id = this.currTab.id + "-dataTD-" + index + "-" + "extPriorityRev";
			td.insertAdjacentHTML("afterBegin", extPriorityRev);
			
			// 运行时间(光纤交换机)
			var td = detailDataTDArray[runtime_cellIndex];
			td.id = this.currTab.id + "-dataTD-" + index + "-" + "runtime";
			td.insertAdjacentHTML("afterBegin", runtime);
	...
	getLightInfo: function (){
		FibreNetRemoteService.getLightInfo(this.nodeid, this.type, this.subtype, {
			callback:function(data){
				ObjectSelf.callbackgetLightInfo(data);
			}
		});
	},
	
	// 获取设备信号灯信息后处理的回调函数
	callbackgetLightInfo: function(data){
		this.data = data;
		this.createLightInfo();
	},
	
	// 获取设备信号灯信息后 创建设备信号灯信息
	createLightInfo: function(){
		var listTBODY = this.detailDataTABLE.tBodies[1];
		if(!listTBODY){
			// 如果详细信息页面数据表格中 数据列表的 TBODY 为 null 则重新创建 (一般不会出现)
			this.createDetailDataTABLE();
		...
		var lightInfoList = this.data;
		var length = lightInfoList.length;
		...
		var color_cellIndex = $(currTabId + "-titleTD-" + "color").cellIndex;
		var type_cellIndex = $(currTabId + "-titleTD-" + "type").cellIndex;
		var state_cellIndex = $(currTabId + "-titleTD-" + "state").cellIndex;
		var displycolor_cellIndex = $(currTabId + "-titleTD-" + "displycolor").cellIndex;
		
		var i = 0;
		while(i < length){
			var lightInfo = lightInfoList[i];
			i++;
			var index = i;									// 序号
			var sindex = lightInfo.sindex;					// 索引
			var descr = lightInfo.descr;					// 描述
			var color = lightInfo.color;					// 信号灯颜色
			var type = lightInfo.type;						// 类型
			var state = lightInfo.state;					// 状态
			var displycolor = lightInfo.displycolor;		// 信号灯显示颜色
			...
			// 信号灯颜色
			var td = detailDataTDArray[color_cellIndex];
			td.id = this.currTab.id + "-dataTD-" + index + "-" + "color";
			td.insertAdjacentHTML("afterBegin", color);
			...
			// 信号灯显示颜色
			var statusImgSrc = "";
        	if("green" == displycolor){
        	    statusImgSrc = "status_ok.gif";
        	}else if("red" == displycolor){
        	    statusImgSrc = "alert.gif";
        	}else if("amber" == displycolor){
        		statusImgSrc = "alarm_level_1.gif";
        	}else{
        		statusImgSrc = "alarm_level_2.gif";
        	}
        	var img = this.createElement("", "img");
        	img.src = this.rootPath + "/resource/image/topo/" + statusImgSrc;
			var td = detailDataTDArray[displycolor_cellIndex];
			td.id = this.currTab.id + "-dataTD-" + index + "-" + "displycolor";
			td.appendChild(img);
	...
	// ------------			设备光口配置信息    START				----------------------
	// 获取设备光口配置信息
	getFibreConfigInfo: function (){
		FibreNetRemoteService.getFibreConfigInfo(this.nodeid, this.type, this.subtype, {
			callback:function(data){
				ObjectSelf.callbackgetgetFibreConfigInfo(data);
			}
		});
	},
	
	// 获取设备光口配置信息后处理的回调函数
	callbackgetgetFibreConfigInfo: function(data){
		this.data = data;
		this.createFibreConfigInfo();
	},
	
	// 获取设备光口配置信息后 创建设备光口配置信息
	createFibreConfigInfo: function(){
		var listTBODY = this.detailDataTABLE.tBodies[1];
		if(!listTBODY){
			// 如果详细信息页面数据表格中 数据列表的 TBODY 为 null 则重新创建 (一般不会出现)
			this.createDetailDataTABLE();
		...
		var fibreConfigInfoList = this.data;
		var length = fibreConfigInfoList.length;
		if(!fibreConfigInfoList || length == 0){
			return;
		}
		
		var currTabId = this.currTab.id;
		var fabricName_cellIndex = $(currTabId + "-titleTD-" + "fabricName").cellIndex;
		var elementName_cellIndex = $(currTabId + "-titleTD-" + "elementName").cellIndex;
		var moduleName_cellIndex = $(currTabId + "-titleTD-" + "moduleName").cellIndex;
		var moduleCapacity_cellIndex = $(currTabId + "-titleTD-" + "moduleCapacity").cellIndex;
		var moduleDescr_cellIndex = $(currTabId + "-titleTD-" + "moduleDescr").cellIndex;
		var feModuleObjectID_cellIndex = $(currTabId + "-titleTD-" + "feModuleObjectID").cellIndex;
		var moduleOperStatus_cellIndex = $(currTabId + "-titleTD-" + "moduleOperStatus").cellIndex;
		var upTime_cellIndex = $(currTabId + "-titleTD-" + "upTime").cellIndex;
		var moduleFxPortCapacity_cellIndex = $(currTabId + "-titleTD-" + "moduleFxPortCapacity").cellIndex;
		
		var i = 0;
		while(i < length){
			var fibreConfigInfo = fibreConfigInfoList[i];
			i++;
			var index = i;														// 序号
			var fabricName = fibreConfigInfo.fabricName;						// 描述
		    var elementName = fibreConfigInfo.elementName;						// 元素名称
	    	var moduleName = fibreConfigInfo.moduleName;						// 模块名称
		    var moduleCapacity = fibreConfigInfo.moduleCapacity;				// 模块数量
		    var moduleDescr = fibreConfigInfo.moduleDescr;						// 模块描述
		    var feModuleObjectID = fibreConfigInfo.feModuleObjectID;			// 模块OID
		    var moduleOperStatus = fibreConfigInfo.moduleOperStatus;			// 模块状态
		    var upTime = fibreConfigInfo.upTime;								// 启动时间
		    var moduleFxPortCapacity = fibreConfigInfo.moduleFxPortCapacity;	// 端口数
			...
			// 描述
			var td = detailDataTDArray[fabricName_cellIndex];
			td.id = this.currTab.id + "-dataTD-" + index + "-" + "fabricName";
			td.insertAdjacentHTML("afterBegin", fabricName);
			
			// 元素名称
			var td = detailDataTDArray[elementName_cellIndex];
			td.id = this.currTab.id + "-dataTD-" + index + "-" + "elementName";
			td.insertAdjacentHTML("afterBegin", elementName);
			
			// 模块名称
			var td = detailDataTDArray[moduleName_cellIndex];
			td.id = this.currTab.id + "-dataTD-" + index + "-" + "moduleName";
			td.insertAdjacentHTML("afterBegin", moduleName);
			
			// 模块数量
			var td = detailDataTDArray[moduleCapacity_cellIndex];
			td.id = this.currTab.id + "-dataTD-" + index + "-" + "moduleCapacity";
			td.insertAdjacentHTML("afterBegin", moduleCapacity);
			
			// 模块描述
			var td = detailDataTDArray[moduleDescr_cellIndex];
			td.id = this.currTab.id + "-dataTD-" + index + "-" + "moduleDescr";
			td.insertAdjacentHTML("afterBegin", moduleDescr);
			
			// 模块OID
			var td = detailDataTDArray[feModuleObjectID_cellIndex];
			td.id = this.currTab.id + "-dataTD-" + index + "-" + "feModuleObjectID";
			td.insertAdjacentHTML("afterBegin", feModuleObjectID);
			
			// 模块状态
			var td = detailDataTDArray[moduleOperStatus_cellIndex];
			td.id = this.currTab.id + "-dataTD-" + index + "-" + "moduleOperStatus";
			td.insertAdjacentHTML("afterBegin", moduleOperStatus);
			
			// 启动时间
			var td = detailDataTDArray[upTime_cellIndex];
			td.id = this.currTab.id + "-dataTD-" + index + "-" + "upTime";
			td.insertAdjacentHTML("afterBegin", upTime);
			
			// 端口数
			var td = detailDataTDArray[moduleFxPortCapacity_cellIndex];
			td.id = this.currTab.id + "-dataTD-" + index + "-" + "moduleFxPortCapacity";
			td.insertAdjacentHTML("afterBegin", moduleFxPortCapacity);
	...
	// ------------			设备光口性能信息    START				----------------------
	// 获取设备光口性能信息
	getFibreCapabilityInfo: function (){
		FibreNetRemoteService.getFibreCapabilityInfo(this.nodeid, this.type, this.subtype, {
			callback:function(data){
				ObjectSelf.callbackgetFibreCapabilityInfo(data);
			}
		});
	},
	
	// 获取设备光口性能信息后处理的回调函数
	callbackgetFibreCapabilityInfo: function(data){
		this.data = data;
		this.createFibreCapabilityInfo();
	},
	
	// 获取设光口性能信息后 创建设备光口性能信息
	createFibreCapabilityInfo: function(){
		var listTBODY = this.detailDataTABLE.tBodies[1];
		if(!listTBODY){
			// 如果详细信息页面数据表格中 数据列表的 TBODY 为 null 则重新创建 (一般不会出现)
			this.createDetailDataTABLE();
		...
		var fibreCapabilityInfoList = this.data;
		var length = fibreCapabilityInfoList.length;
		if(!fibreCapabilityInfoList || length == 0){
			return;
		}
		
		var currTabId = this.currTab.id;
		var portName_cellIndex = $(currTabId + "-titleTD-" + "portName").cellIndex;
		var portPhysOperStatus_cellIndex = $(currTabId + "-titleTD-" + "portPhysOperStatus").cellIndex;
		var portC3InFrames_cellIndex = $(currTabId + "-titleTD-" + "portC3InFrames").cellIndex;
		var portC3OutFrames_cellIndex = $(currTabId + "-titleTD-" + "portC3OutFrames").cellIndex;
		var portC3InOctets_cellIndex = $(currTabId + "-titleTD-" + "portC3InOctets").cellIndex;
		var portC3OutOctets_cellIndex = $(currTabId + "-titleTD-" + "portC3OutOctets").cellIndex;
		var portC3Discards_cellIndex = $(currTabId + "-titleTD-" + "portC3Discards").cellIndex;
		
		var i = 0;
		while(i < length){
			var fibreCapabilityInfo = fibreCapabilityInfoList[i];
			i++;
			var index = i;														// 序号
			var portName = fibreCapabilityInfo.portName;						// 光口名称
			var portPhysOperStatus = fibreCapabilityInfo.portPhysOperStatus;	// 当前状态
			var portC3InFrames = fibreCapabilityInfo.portC3InFrames;			// 入口帧数
			var portC3OutFrames = fibreCapabilityInfo.portC3OutFrames;			// 出口帧数
			var portC3InOctets = fibreCapabilityInfo.portC3InOctets;			// 入口帧字节数
			var portC3OutOctets = fibreCapabilityInfo.portC3OutOctets;			// 出口帧字节数
			var portC3Discards = fibreCapabilityInfo.portC3Discards;			// 丢包数
			...
			// 光口名称
			var td = detailDataTDArray[portName_cellIndex];
			td.id = this.currTab.id + "-dataTD-" + index + "-" + "portName";
			td.insertAdjacentHTML("afterBegin", portName);
			
			// 当前状态
			var td = detailDataTDArray[portPhysOperStatus_cellIndex];
			td.id = this.currTab.id + "-dataTD-" + index + "-" + "portPhysOperStatus";
			td.insertAdjacentHTML("afterBegin", portPhysOperStatus);
			
			// 入口帧数
			var td = detailDataTDArray[portC3InFrames_cellIndex];
			td.id = this.currTab.id + "-dataTD-" + index + "-" + "portC3InFrames";
			td.insertAdjacentHTML("afterBegin", portC3InFrames);
			tr.appendChild(td);
			
			// 出口帧数
			var td = detailDataTDArray[portC3OutFrames_cellIndex];
			td.id = this.currTab.id + "-dataTD-" + index + "-" + "portC3OutFrames";
			td.insertAdjacentHTML("afterBegin", portC3OutFrames);
			
			// 入口帧字节数
			var td = detailDataTDArray[portC3InOctets_cellIndex];
			td.id = this.currTab.id + "-dataTD-" + index + "-" + "portC3InOctets";
			td.insertAdjacentHTML("afterBegin", portC3InOctets);
			
			// 出口帧字节数
			var td = detailDataTDArray[portC3OutOctets_cellIndex];
			td.id = this.currTab.id + "-dataTD-" + index + "-" + "portC3OutOctets";
			td.insertAdjacentHTML("afterBegin", portC3OutOctets);
			
			// 丢包数
			var td = detailDataTDArray[portC3Discards_cellIndex];
			td.id = this.currTab.id + "-dataTD-" + index + "-" + "portC3Discards";
			td.insertAdjacentHTML("afterBegin", portC3Discards);
		}
	...
	getAlarmInfo: function(){
		...
		FibreNetRemoteService.getAlarmInfo(this.nodeid, this.type, this.subtype, 
	...
	showAlarmDetailInfo: function(eventlocation, subentity){
		...
		FibreNetRemoteService.getAlarmDetailInfo(this.nodeid, this.type, this.subtype, 
