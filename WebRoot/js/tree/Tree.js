...
function Tree(htmlObject,width,height,rootId)
{
	...
	this.htmlNode=new TreeItem(this.rootId,"",0,this);
	...
	this.allTree.onselectstart=new Function("return true;");	// FIXME
	this.XMLLoader=new XMLLoader(this._parseXMLTree,this,true,this.no_cashe);
...
function TreeItem(itemId,itemText,parentObject,treeObject,actionHandler,mode)
{
	this.htmlNode="";
	this.acolor="";
	this.scolor="";
 ...
 Tree.prototype._globalIdStorageAdd=function(itemId,itemObject){
 ...
 Tree.prototype._globalIdStorageSub=function(itemId){
 for(var i=0;i<this._globalIdStorageSize;i++)
 if(this._globalIdStorage[i]==itemId)
{
 ...
 Tree.prototype._globalIdStorageFind=function(itemId,skipXMLSearch,skipParsing,isreparse){
 ...
 if(this._globalIdStorage[i]==itemId)
{
 ...
 Tree.prototype._escape=function(str){
 switch(this.utfesc){
 case "none":
 return str;
 break;
 ...
 Tree.prototype._drawNewTr=function(htmlObject,node)
{
 ...
 Tree.prototype.loadXMLString=function(xmlString,afterCall){
 var that=this;
 ...
 Tree.prototype.loadXML=function(file,afterCall){
 var that=this;
 ...
 Tree.prototype._attachChildNode=function(parentObject,itemId,itemText,itemActionHandler,image1,image2,image3,optionStr,childs,beforeNode,afterNode){
 ...
 switch(tempStr[i])
{
 ...
 Nodes[Count]=new TreeItem(itemId,itemText,parentObject,this,itemActionHandler,1);
 ...
 if(parentObject.childsCount>=2)
{
 ...
 Tree.prototype.insertNewItem=function(parentId,itemId,itemText,itemActionHandler,image1,image2,image3,optionStr,childs){
 ...
 Tree.prototype.insertNewChild=function(parentId,itemId,itemText,itemActionHandler,image1,image2,image3,optionStr,childs){
 return this.insertNewItem(parentId,itemId,itemText,itemActionHandler,image1,image2,image3,optionStr,childs);
}
 
 Tree.prototype._parseXMLTree=function(dhtmlObject,node,parentId,level,xml_obj,start){
 ...
 if((node.childNodes[i].nodeType==1)&&(node.childNodes[i].tagName == "item"))
{
 ...
 Tree.prototype.checkUserData=function(node,parentId){
 if((node.nodeType==1)&&(node.tagName == "userdata"))
{
 ...
 Tree.prototype._redrawFrom=function(dhtmlObject,itemObject,start,visMode){
 if(!itemObject){
 ...
 if(tempx.childNodes[i].openMe==1)
{
 ...
 Tree.prototype._createSelf=function(){
 ...
 Tree.prototype._xcloseAll=function(itemObject)
{
 ...
 Tree.prototype._xopenAll=function(itemObject)
{
 ...
 Tree.prototype._correctPlus=function(itemObject){
 ...
 else
{
 ...
 Tree.prototype._correctLine=function(itemObject){
 ...
 for(var i=1;i<=itemObject.childsCount;i++)
{
 ...
 Tree.prototype._getCountStatus=function(itemId,itemObject){
 ...
 Tree.prototype._getLineStatus =function(itemId,itemObject){
 if(itemObject.childNodes[itemObject.childsCount-1].id==itemId)return 0;
 return 1;
}

 
 Tree.prototype._HideShow=function(itemObject,mode){
 if((this.XMLsource)&&(!itemObject.XMLload)){
 if(mode==1)return;
 ...
 Tree.prototype._getOpenState=function(itemObject){
 ...
 Tree.prototype.onRowClick2=function(){
 ...
 Tree.prototype.onRowClick=function(){
 ...
 Tree.prototype._epnFHe=function(that,id){
 if(that._epnFH)
 ...
 Tree.prototype.onRowClickDown=function(e){
 e=e||window.event;
 var that=this.parentObject.treeNod;
...
Tree.prototype.getSelectedItemId = function()
{
 ...
 Tree.prototype._selectItem=function(node,e){

 this._unselectItems();

 this._markItem(node);
}
 Tree.prototype._markItem=function(node){
 ...
 Tree.prototype.getIndexById=function(itemId){
 var z=this._globalIdStorageFind(itemId);
 if(!z)return null;
 return this._getIndex(z);
};
 Tree.prototype._getIndex=function(w){
 ...
 Tree.prototype._unselectItem=function(node){
 if((node)&&(node.i_sel))
{
 ...
 Tree.prototype._unselectItems=function(){
 for(var i=0;i<this._selected.length;i++){
 ...
 Tree.prototype.onRowSelect=function(e,htmlObject,mode){
...
Tree.prototype._correctCheckStates=function(dhtmlObject){
...
Tree.prototype._correctChildrenCheckStates=function(dhtmlObject){
 //if(!this.tscheck)return;
 if(dhtmlObject.id==this.rootId)return;
 var act=dhtmlObject.childNodes;
 var flag1=0;var flag2=0;
 ...
 if(dhtmlObject.checkstate==0)flag1=1;
 else if(dhtmlObject.checkstate==1)flag2=1;
 if(dhtmlObject.checkstate==2){flag1=1;flag2=1;}
 
 for(var i=0;i<dhtmlObject.childsCount;i++){

 if((flag1)&&(flag2))this._setCheck(act[i],"unsure");//非全选
 else if(flag1)this._setCheck(act[i],false);
 else this._setCheck(act[i],true);
 this._correctChildrenCheckStates(act[i]);
}
...
Tree.prototype.checkedLabel=function(dhtmlObject){

	if(dhtmlObject.id==this.rootId)return;
	var act=dhtmlObject.childNodes;
	var length=this._checked.length;//数组长度 1
	if(dhtmlObject.childsCount==0)  //叶子
	{
		if(dhtmlObject.checkstate>0)//选中 1 叶子 2 父节点并且子节点没有全部被选中。
		{
			this._checked[length] = dhtmlObject.label;//从1开始放选中的叶子
		}
		else
		{
			if(this._checked.length<2)
			{
				this._checked = new Array(1);				
			}
			else
			{
			    ...
			    var j=1;
				var temp="";
				for(var i=1;i<this._checked.length;i++)
				{
					if(dhtmlObject.label!=this._checked[i])
					{
						tempArray[j]=this._checked[i];
						j++;
				...
				this._checked = tempArray;
			}			
		}
		if(this._checked.length>1)this._checked[0]=this._checked.length-1;  //_checked数组0位放选中叶子个数
		else this._checked[0]=0;	
	}
	else                              //父节点
	{
		for(var i=0;i<dhtmlObject.childsCount;i++)
		{
			this.checkedLabel(act[i]);
		}
	}
}
 
 Tree.prototype.getCheckedArray=function(){
	 var returnArray = new Array();
	 returnArray = this._checked;
	 return returnArray;
 }
 
 Tree.prototype.onCheckBoxClick=function(e){
 ...
 this.treeNod._correctChildrenCheckStates(this.parentObject);

 this.treeNod.checkedLabel(this.parentObject);
 
 if(this.treeNod.checkFuncHandler)return(this.treeNod.checkFuncHandler(this.parentObject.id,this.parentObject.checkstate));
 else return true;
};
 
 Tree.prototype._createItem=function(acheck,itemObject,mode){
 ...
 else
{
 ...
 Tree.prototype.setImagePath=function(newPath){this.imPath=newPath;};



 
 Tree.prototype.setOnRightClickHandler=function(func){if(typeof(func)=="function")this.arFunc=func;else this.arFunc=eval(func);};

 
 Tree.prototype.setOnClickHandler=function(func){if(typeof(func)=="function")this.aFunc=func;else this.aFunc=eval(func);};


 
 Tree.prototype.setXMLAutoLoading=function(filePath){this.XMLsource=filePath;};

 
 Tree.prototype.setOnCheckHandler=function(func){if(typeof(func)=="function")this.checkFuncHandler=func;else this.checkFuncHandler=eval(func);};


 
 Tree.prototype.setOnOpenHandler=function(func){if(typeof(func)=="function")this._spnFH=func;else this._spnFH=eval(func);};
 
 Tree.prototype.setOnOpenStartHandler=function(func){if(typeof(func)=="function")this._spnFH=func;else this._spnFH=eval(func);};

 
 Tree.prototype.setOnOpenEndHandler=function(func){if(typeof(func)=="function")this._epnFH=func;else this._epnFH=eval(func);};

 
 Tree.prototype.setOnDblClickHandler=function(func){if(typeof(func)=="function")this.dblclickFuncHandler=func;else this.dblclickFuncHandler=eval(func);};
 ...
 Tree.prototype.openAllItems=function(itemId)
{
 ...
 Tree.prototype.getOpenState=function(itemId){
 var temp=this._globalIdStorageFind(itemId);
 if(!temp)return "";
 ...
 Tree.prototype.closeAllItems=function(itemId)
{
 ...
 Tree.prototype.setUserData=function(itemId,name,value){
 ...
 Tree.prototype.getUserData=function(itemId,name){
 var sNode=this._globalIdStorageFind(itemId,0,true);
 if(!sNode)return;
 ...
 Tree.prototype.getItemColor=function(itemId)
{
 var temp=this._globalIdStorageFind(itemId);
 if(!temp)return 0;
 ...
 Tree.prototype.setItemColor=function(itemId,defaultColor,selectedColor)
{
...
{if(defaultColor)temp.span.style.color=defaultColor;}
 ...
 Tree.prototype.getItemText=function(itemId)
{
 var temp=this._globalIdStorageFind(itemId);
 if(!temp)return 0;
 ...
 Tree.prototype.getParentId=function(itemId)
{
 var temp=this._globalIdStorageFind(itemId);
 ...
 Tree.prototype.changeItemId=function(itemId,newItemId)
{
 var temp=this._globalIdStorageFind(itemId);
 if(!temp)return 0;
 temp.id=newItemId;
 ...
 if(this._globalIdStorage[i]==itemId)
{
 ...
 Tree.prototype.doPaste=function(itemId){
 ...
 Tree.prototype.clearCut=function(){
 for(var i=0;i<this.nodeCut.length;i++)
{
 var tempa=this.nodeCut[i];
 ...
 Tree.prototype._moveNode=function(itemObject,targetObject){
...
Tree.prototype._fixNodesCollection=function(target,zParent){
...
Tree.prototype._recreateBranch=function(itemObject,targetObject,beforeNode,level){
 var i;var st="";
 if(beforeNode){
 ...
 Tree.prototype._moveNodeTo=function(itemObject,targetObject,beforeNode){
 ...
 if(targetObject.childsCount>=2)
{
 ...
 Tree.prototype._clearStyles=function(itemObject){
 ...
 Tree.prototype._registerBranch=function(itemObject,oldTree){
 ...
 Tree.prototype.enableThreeStateCheckboxes=function(mode){this.tscheck=convertStringToBoolean(mode);};


 
 Tree.prototype.setOnMouseInHandler=function(func){
 this.ehlt=true;
 if(typeof(func)=="function")this._onMSI=func;else this.aFunc=eval(func);};

 
 Tree.prototype.setOnMouseOutHandler=function(func){
 this.ehlt=true;
 ...
 Tree.prototype.enableTreeImages=function(mode){this.timgen=convertStringToBoolean(mode);};
 

 
 
 Tree.prototype.enableFixedMode=function(mode){this.hfMode=convertStringToBoolean(mode);};
 
 
 Tree.prototype.enableCheckBoxes=function(mode,hidden){this.checkBoxOff=convertStringToBoolean(mode);this.cBROf=(!(this.checkBoxOff||convertStringToBoolean(hidden)));};
 
 Tree.prototype.setStdImages=function(image1,image2,image3){
 this.imageArray[0]=image1;this.imageArray[1]=image2;this.imageArray[2]=image3;};

 
 Tree.prototype.enableTreeLines=function(mode){
 this.treeLinesOn=convertStringToBoolean(mode);
}

 
 Tree.prototype.setImageArrays=function(arrayName,image1,image2,image3,image4,image5){
 switch(arrayName){
 ...
 Tree.prototype.openItem=function(itemId){
 var temp=this._globalIdStorageFind(itemId);
 if(!temp)return 0;
 ...
 Tree.prototype._openItem=function(item){
 ...
 Tree.prototype.closeItem=function(itemId){
 ...
 Tree.prototype.getLevel=function(itemId){
 var temp=this._globalIdStorageFind(itemId);
 if(!temp)return 0;
 ...
 Tree.prototype.setItemCloseable=function(itemId,flag)
{
 ...
 Tree.prototype._getNodeLevel=function(itemObject,count){
 if(itemObject.parentObject)return this._getNodeLevel(itemObject.parentObject,count+1);
 return(count);
};
 
 
 Tree.prototype.hasChildren=function(itemId){
 var temp=this._globalIdStorageFind(itemId);
 if(!temp)return 0;
 else 
{
 ...
 Tree.prototype._getLeafCount=function(itemNode){
 var a=0;
 ...
 Tree.prototype.setItemText=function(itemId,newLabel,newTooltip)
{
 var temp=this._globalIdStorageFind(itemId);
 if(!temp)return 0;
 temp.label=newLabel;
 ...
 Tree.prototype.getItemTooltip=function(itemId){
 var temp=this._globalIdStorageFind(itemId);
 if(!temp)return "";
 ...
 Tree.prototype.refreshItem=function(itemId){
 ...
 Tree.prototype.setItemImage2=function(itemId,image1,image2,image3){
 var temp=this._globalIdStorageFind(itemId);
 if(!temp)return 0;
 ...
 Tree.prototype.setItemImage=function(itemId,image1,image2)
{
 var temp=this._globalIdStorageFind(itemId);
 if(!temp)return 0;
 if(image2)
{
 ...
 Tree.prototype.getSubItems =function(itemId)
{
 ...
 Tree.prototype._getAllScraggyItems =function(node)
{
 var z="";
 ...
 if((node.childNodes[i].unParsed)||(node.childNodes[i].childsCount>0))
{
 ...
 Tree.prototype._getAllFatItems =function(node)
{
 var z="";
 for(var i=0;i<node.childsCount;i++)
{
 if((node.childNodes[i].unParsed)||(node.childNodes[i].childsCount>0))
{
 ...
 Tree.prototype._getAllSubItems =function(itemId,z,node)
{
 if(node)temp=node;
 else{
 var temp=this._globalIdStorageFind(itemId);
 ...
 for(var i=0;i<temp.childsCount;i++)
{
 ...
 Tree.prototype.selectItem=function(itemId,mode,preserve){
 ...
 Tree.prototype.getSelectedItemText=function()
{
 var str=new Array();
 ...
 Tree.prototype._compressChildList=function(Count,Nodes)
{
 Count--;
 for(var i=0;i<Count;i++)
{
 ...
 Tree.prototype._deleteNode=function(itemId,htmlObject,skip){
 ...
 for(var i=0;i<Count;i++)
{
 ...
 Tree.prototype.setCheck=function(itemId,state){
 ...
 else
{
 ...
 Tree.prototype._setCheck=function(sNode,state){
...
Tree.prototype.setSubChecked=function(itemId,state){
 ...
 Tree.prototype._setSubChecked=function(state,sNode){
 state=convertStringToBoolean(state);
 if(!sNode)return;
 if(((sNode.parentObject._r_logic)||(this._frbtr))&&(state))
 for(var i=0;i<sNode.parentObject.childsCount;i++)
 ...
 for(var i=0;i<sNode.childsCount;i++)
{
 ...
 Tree.prototype.isItemChecked=function(itemId){
 var sNode=this._globalIdStorageFind(itemId);
 if(!sNode)return;
 ...
 Tree.prototype.deleteChildItems=function(itemId)
{
 var sNode=this._globalIdStorageFind(itemId);
 if(!sNode)return;
...
Tree.prototype.deleteItem=function(itemId,selectParent){
...
Tree.prototype._deleteItem=function(itemId,selectParent,skip){
 ...
 Tree.prototype._globalIdStorageRecSub=function(itemObject){
 for(var i=0;i<itemObject.childsCount;i++)
{
 ...
 Tree.prototype.insertNewNext=function(itemId,newItemId,itemText,itemActionHandler,image1,image2,image3,optionStr,childs){
 var sNode=this._globalIdStorageFind(itemId);
 ...
 Tree.prototype.getItemIdByIndex=function(itemId,index){
 var z=this._globalIdStorageFind(itemId);
 ...
 Tree.prototype.getChildItemIdByIndex=function(itemId,index){
 var z=this._globalIdStorageFind(itemId);
 if((!z)||(index>=z.childsCount))return null;
 return z.childNodes[index].id;
};
 ...
 Tree.prototype.setDragHandler=function(func){if(typeof(func)=="function")this.dragFunc=func;else this.dragFunc=eval(func);};
 
 
 Tree.prototype._clearMove=function(){
 if(this._lastMark){
 ...
 Tree.prototype.enableDragAndDrop=function(mode,rmode){
 ...
 Tree.prototype._setMove=function(htmlNode,x,y){
...
Tree.prototype._createDragNode=function(htmlObject,e){
...
Tree.prototype._focusNode=function(item){
...
Tree.prototype._preventNsDrag=function(e){
 if((e)&&(e.preventDefault)){e.preventDefault();return false;}
 return false;
}

Tree.prototype._drag=function(sourceHtmlObject,dhtmlObject,targetHtmlObject){
...
Tree.prototype._dragIn=function(htmlObject,shtmlObject,x,y){
...
Tree.prototype._autoOpenItem=function(e,treeObject){
 treeObject.openItem(treeObject._autoOpenId);
};
Tree.prototype._dragOut=function(htmlObject){
this._clearMove();
if(this._autoOpenTimer)clearTimeout(this._autoOpenTimer);
}
 ...
 Tree.prototype._loadDynXML=function(id,src){
 ...
 Tree.prototype._checkPNodes=function(item1,item2){
...
Tree.prototype.preventIECashing=function(mode){
 ...
 Tree.prototype.setEscapingMode=function(mode){
 this.utfesc=mode;
}


 
 Tree.prototype.enableHighlighting=function(mode){this.ehlt=true;this.ehlta=convertStringToBoolean(mode);};

 
 Tree.prototype._itemMouseOut=function(){
 ...
 Tree.prototype._itemMouseIn=function(){
 var that=this.childNodes[3].parentObject;
 var tree=that.treeNod;
 ...
 Tree.prototype.enableActiveImages=function(mode){this._aimgs=convertStringToBoolean(mode);};

 
Tree.prototype.focusItem=function(itemId){
 var sNode=this._globalIdStorageFind(itemId);
 if(!sNode)return(0);
 ...
 Tree.prototype.getAllSubItems =function(itemId){
 return this._getAllSubItems(itemId);
}

 
 Tree.prototype.getAllChildless =function(){
 return this._getAllScraggyItems(this.htmlNode);
}
 Tree.prototype.getAllLeafs=Tree.prototype.getAllChildless;
 ...
 if((node.childNodes[i].unParsed)||(node.childNodes[i].childsCount>0))
{
 ...
 Tree.prototype.getAllItemsWithKids =function(){
 return this._getAllFatItems(this.htmlNode);
}
 Tree.prototype.getAllFatItems=Tree.prototype.getAllItemsWithKids;



 
 Tree.prototype.getAllChecked=function(){
 return this._getAllChecked("","",1);
}
 
 Tree.prototype.getAllUnchecked=function(itemId){
 if(itemId)
 ...
 Tree.prototype.getAllPartiallyChecked=function(){
 return this._getAllChecked("","",2);
}


 
 Tree.prototype.getAllCheckedBranches=function(){
 ...
 Tree.prototype._getAllChecked=function(htmlNode,list,mode){
...
Tree.prototype.setItemStyle=function(itemId,style_string){
 var temp=this._globalIdStorageFind(itemId);
 if(!temp)return 0;
...
Tree.prototype.enableImageDrag=function(mode){
 this._itim_dg=convertStringToBoolean(mode);
}

 
 Tree.prototype.setOnDragIn=function(func){
 if(typeof(func)=="function")this._onDrInFunc=func;else this._onDrInFunc=eval(func);
};

 
 Tree.prototype.enableDragAndDropScrolling=function(mode){this.autoScroll=convertStringToBoolean(mode);};

 Tree.prototype.setDelimiter= function(delim)
{
this.dlmtr = delim;
} 
