...
function tipsWindown(title,content,width,height,drag,time,showbg,cssName,backcall) {
	...
	if(showWindown == true) {
		var simpleWindown_html = new String;
			simpleWindown_html = "<div id=\"windownbg\" style=\"height:"+$(document).height()+"px;filter:alpha(opacity=70);opacity:70;z-index: 999901\"></div>";
	...
	$("#windownbg").animate({opacity:"0.9"},"normal");//设置透明度
	$("#windown-box").show();
	if( height >= 527 ) {
	...
	if ( _version == 6.0 ) {
		...
		$("#windown-box").css({position:"absolute", top: "0px",right:"150px"});
	}else {
		//$("#windown-box").css({left:"50%",top:"50%",marginTop:-((parseInt(height)+53)/2)+"px",marginLeft:-((parseInt(width)+32)/2)+"px",zIndex: "999999"});
		$("#windown-box").css({position:"absolute", top: "0px",right:"150px"});
	};
