...
<%
	...
	String level_desc = (String)request.getAttribute("level_desc");	
...
setTimeout(function(){
 ...
 Ext.get("process").on("click",function(){
  
     {
        	...
        	mainForm.action = "<%=rootPath%>/menu.do?action=edit";
        	mainForm.submit();
						...
						<tr>
							<td class="td-container-main-add">
								<table id="container-main-add" class="container-main-add">
									...
									<td width="30%">&nbsp;<input type="text" id="ch_desc" name="ch_desc" value="<%=function.getCh_desc() %>"><font color="red">&nbsp;*</font></td>
								...
								<tr >
									...
									<td width="30%">&nbsp;<input type="text" id="width" name="width" value="<%=function.getWidth() %>"></td>
									<td align="right" height="24" width="20%">弹出窗口高度&nbsp;</td>
									<td width="30%">&nbsp;<input type="text" id="height" name="height" value="<%=function.getHeight() %>"></td>
									
								</tr>	
								<tr >
									<td align="right" height="24" width="20%">弹出窗口左边距离&nbsp;</td>
									<td width="30%">&nbsp;<input type="text" id="clientX" name="clientX" value="<%=function.getClientX() %>"></td>
									<td align="right" height="24" width="20%">弹出窗口上边距离&nbsp;</td>
									<td width="30%">&nbsp;<input type="text" id="clientY" name="clientY" value="<%=function.getClientY() %>"></td>
									
								</tr>
								
								<tr id="url&tr" >
									<td align="right" height="24" width="20%">菜单URL：<%=rootPath %>/&nbsp;</td>
									<td width="30%">&nbsp;<input type="text" id="url" name="url" value="<%=function.getUrl() %>"><font color="red">&nbsp;*</font></td>
									<td align="right" height="24" width="20%">菜单图片URL：<%=rootPath %>/&nbsp;</td>
									<td width="30%">&nbsp;<input type="text" id="img_url" name="img_url" value="<%=function.getImg_url() %>"><font color="red">&nbsp;*</font></td>
									
								</tr>
								
								<tr>
									<td><input type="hidden" id="id" name="id" value="<%=function.getId()%>"></td>						
								</tr>											                 										                      								            							
															<tr>
...
<script>
	function defaultSelect(){
		document.all.isCurrentWindow.value = <%=function.getIsCurrentWindow()%>;
		document.all.level_desc.value = <%=function.getLevel_desc()%>;
	}
	defaultSelect();
</script>
...
<script>
	...
	function unionFaterhMenuSelect(){
		...
		%>
			if(level_desc == <%=allFunction.get(i).getLevel_desc()+1%>&&<%=function.getId()%>!=<%=allFunction.get(i).getId()%>){
		...
		for(var j =0 ; j<functionFlag;j++){
			...
			if(fatherNodeArray[j][1]=="<%=function.getFather_node()%>"){
				fatherNode.value =<%=function.getFather_node()%>;
			}	
		}
		unionUrl();
	}
