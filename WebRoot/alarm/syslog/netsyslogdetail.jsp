...
<%@page import="com.afunms.event.model.NetSyslogEvent"%>
...
<%
	String priorityName="",ipaddress="",hostname="",message="";
	Date cc = null;
	Object obj = request.getAttribute("syslog");
	if(obj instanceof NetSyslogEvent){
		NetSyslogEvent syslog = (NetSyslogEvent)obj;
		priorityName = syslog.getPriorityName();
		ipaddress = syslog.getIpaddress();
		hostname = syslog.getHostname();
		message = syslog.getMessage();
		cc = syslog.getRecordtime().getTime();
	}else if(obj instanceof NetSyslog){
		NetSyslog syslog = (NetSyslog)obj;
		priorityName = syslog.getPriorityName();
		ipaddress = syslog.getIpaddress();
		hostname = syslog.getHostname();
		message = syslog.getMessage();
...
<INPUT type="button" value="关闭窗口" id=button1 name=button1 onclick="window.close()" class="button">            
  ...
  <tr bgcolor=#cedefa height=25>
    ...
    <td colspan=4>描述(D):</td>
	 ...
	 <tr >
    <td colspan=4><textarea  rows="10" cols="50"><%=message %>
    	</textarea></td>
  </tr>
