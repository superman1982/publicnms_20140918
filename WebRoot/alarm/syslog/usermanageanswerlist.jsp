									...
									<table cellspacing="1" cellpadding="0" width="100%">
										...
										<%
												...
												while (iter.hasNext()) {
													...
													Map userMap = (HashMap)entry.getValue();
													Set userSet  = userMap.entrySet();
													Iterator userIter = userSet.iterator();
													while(userIter.hasNext()){
														Map.Entry userEntry = (Map.Entry)userIter.next();
														String username = (String)userEntry.getKey();
														int count = (Integer)userEntry.getValue();
										%>
										...
										<tr bgcolor="#FFFFFF" <%=onmouseoverstyle%> height=25 align="center">
											...
											<td><%=username %></td>
											<td>
												<a href="<%=rootPath%>/netsyslog.do?action=syslogimpevtcatelist&ipaddress=<%=ipaddress%>&streventname=<%=streventname %>&username=<%=username %>&startdate=<%=startdate %>&todate=<%=todate %>&busi=userAccout&jp=1"><%=count%></a>
											</td>
										</tr>
										<%			}
												}
