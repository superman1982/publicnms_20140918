...
public class Emailmonitor_realtimeDao extends BaseDao implements DaoInterface
{
	SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
   public Emailmonitor_realtimeDao()
   {
   ...
   {
	   Emailmonitor_realtime vo = (Emailmonitor_realtime)baseVo;	  
	...
	public Hashtable getAllReal() throws Exception {
		...
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				Emailmonitor_realtime emailmonitor_realtime = (Emailmonitor_realtime) list.get(i);
   ...
   {
	   Emailmonitor_realtime vo = new Emailmonitor_realtime();
       try
       {
