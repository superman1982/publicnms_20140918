...
public class FtpHistoryDao extends BaseDao implements DaoInterface
{
	SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
   public FtpHistoryDao()
   {
   ...
   {
	   FtpHistory vo = (FtpHistory)baseVo;	  
	...
	public Vector getByFTPid(Integer ftpid,String starttime,String totime,Integer isconnected) throws Exception{
    	...
    	try{
    		...
    		if("mysql".equalsIgnoreCase(SystemConstant.DBType)){
    			...
    			sql="select a.is_canconnected,a.reason,a.mon_time from nms_ftp_history a where " +
    			"a.ftp_id="+ftpid+" and (a.mon_time >= '"+starttime +"' and  a.mon_time <= '"+totime+"')";
    		}else if("oracle".equalsIgnoreCase(SystemConstant.DBType)){
    			sql="select a.is_canconnected,a.reason,a.mon_time from nms_ftp_history a where " +
				"a.ftp_id="+ftpid+" and (a.mon_time >= "+"to_date('"+starttime+"','YYYY-MM-DD HH24:MI:SS')"+" and  a.mon_time <= "+"to_date('"+totime+"','YYYY-MM-DD HH24:MI:SS'))";
    		}
	...
	public String[] getAvailability(Integer ftp_id,String starttime,String totime,String type)throws Exception{
		...
		try {
			...
			if("mysql".equalsIgnoreCase(SystemConstant.DBType)){
				...
				parm=parm+totime;
				parm=parm+"'";
				sql = "select sum(aa."+type+") as stype ,COUNT(aa.ftp_id) as countid from nms_ftp_history aa where aa.ftp_id="+ftp_id+" and "+parm;
			}else if("oracle".equalsIgnoreCase(SystemConstant.DBType)){
				String parm=" aa.mon_time >= ";
   ...
   public Hashtable getPingData(Integer ftp_id,String starttime,String endtime) {
		...
		if (!starttime.equals("") && !endtime.equals("")) {
			...
			if("mysql".equalsIgnoreCase(SystemConstant.DBType)){
				...
				sql = "select a.is_canconnected,a.reason,a.mon_time from nms_ftp_history a where " +
					"a.ftp_id="+ftp_id+" and (a.mon_time >= "+"to_date('"+starttime+"','YYYY-MM-DD HH24:MI:SS')"+" and  a.mon_time <= "+"to_date('"+endtime+"','YYYY-MM-DD HH24:MI:SS')"+")";;
			}
//			SysLogger.info(sql);
			int i = 0;
			double avgput1 = 0;
   ...
   public Hashtable getPingDataById(Integer ftp_id,String starttime,String endtime) {
	   ...
	   if (!starttime.equals("") && !endtime.equals("")) {
		   ...
		   if("mysql".equalsIgnoreCase(SystemConstant.DBType)){
			   ...
			   sql = "select a.is_canconnected,a.reason,a.mon_time from nms_ftp_history a where " +
			   	"a.ftp_id="+ftp_id+" and (a.mon_time >= "+"to_date('"+starttime+"','YYYY-MM-DD HH24:MI:SS')"+" and  a.mon_time <= "+"to_date('"+endtime+"','YYYY-MM-DD HH24:MI:SS')"+") order by id";;
		   }
//		   SysLogger.info(sql);
		   int i = 0;
		   double curPing=0;
