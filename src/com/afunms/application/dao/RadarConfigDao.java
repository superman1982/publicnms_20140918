...
public class RadarConfigDao extends BaseDao implements DaoInterface {

	public RadarConfigDao() {
		super("nms_radarconfig");
	}
	public boolean delete(String []ids){
	   ...
	   public List getGrapesByFlag(int flag){
		   ...
		   sql.append("select * from nms_radarconfig where mon_flag = "+flag);
	...
	public boolean save(BaseVo vo) {
		RadarConfig vo1=(RadarConfig)vo;
		StringBuffer sql=new StringBuffer();
		sql.append("insert into nms_radarconfig(name,ipaddress,supperdir,subdir,inter,filesize,sendmobiles,mon_flag,netid,sendemail) values('");
	...
	public boolean update(BaseVo vo) {
		RadarConfig vo1=(RadarConfig)vo;
		StringBuffer sql=new StringBuffer();
		sql.append("update nms_radarconfig set name='");
		sql.append(vo1.getName());
	   ...
	   public List getRadarByBID(Vector bids){
		   List rlist = new ArrayList();
		   StringBuffer sql = new StringBuffer();
		   String wstr = "";
		   ...
		   sql.append("select * from nms_radarconfig "+wstr);
		   //SysLogger.info(sql.toString());
		   return findByCriteria(sql.toString());
	   }
	   
	   public RadarConfig getRadarByIp(String ip){
		   RadarConfig config = null;
		   List list = new ArrayList();
		   StringBuffer sql = new StringBuffer();
		   sql.append("select * from nms_radarconfig where ipaddress='"+ip+"'");
		   list = findByCriteria(sql.toString());
		   if(list != null && list.size()>0){
			   config = (RadarConfig)list.get(0);
		   }
		   //SysLogger.info(sql.toString());
		   return config;
	   }
