	...
	public Vector getByTFTPid(Integer ftpid,String starttime,String totime,Integer isconnected) throws Exception{
    	...
    	try{
    		...
    		if("mysql".equalsIgnoreCase(SystemConstant.DBType)){
    			...
    			sql="select a.is_canconnected,a.reason,a.mon_time from nms_tftp_history a where " +
    			"a.tftp_id="+ftpid+" and (a.mon_time >= '"+starttime +"' and  a.mon_time <= '"+totime+"')";
    		}else if("oracle".equalsIgnoreCase(SystemConstant.DBType)){
    			sql="select a.is_canconnected,a.reason,a.mon_time from nms_tftp_history a where " +
				"a.tftp_id="+ftpid+" and (a.mon_time >= "+"to_date('"+starttime+"','YYYY-MM-DD HH24:MI:SS')"+" and  a.mon_time <= "+"to_date('"+totime+"','YYYY-MM-DD HH24:MI:SS'))";
    		}
...
//			System.out.println(sql);
   ...
   public Hashtable getPingData(Integer tftp_id,String starttime,String endtime) {
		...
		if (!starttime.equals("") && !endtime.equals("")) {
			...
			if("mysql".equalsIgnoreCase(SystemConstant.DBType)){
				sql = "select a.is_canconnected,a.reason,a.mon_time from nms_tftp_history a where " +
					"a.tftp_id="+tftp_id+" and (a.mon_time >= '"+starttime +"' and  a.mon_time <= '"+endtime+"')";;
			}else if("oracle".equalsIgnoreCase(SystemConstant.DBType)){
				sql = "select a.is_canconnected,a.reason,a.mon_time from nms_tftp_history a where " +
					"a.tftp_id="+tftp_id+" and (a.mon_time >= "+"to_date('"+starttime+"','YYYY-MM-DD HH24:MI:SS')"+" and  a.mon_time <= "+"to_date('"+endtime+"','YYYY-MM-DD HH24:MI:SS')"+")";;
			}
			SysLogger.info(sql);
			int i = 0;
			double avgput1 = 0;
   ...
   public Hashtable getPingDataById(Integer tftp_id,String starttime,String endtime) {
	   ...
	   if (!starttime.equals("") && !endtime.equals("")) {
		   ...
		   if("mysql".equalsIgnoreCase(SystemConstant.DBType)){
			   sql = "select a.is_canconnected,a.reason,a.mon_time from nms_tftp_history a where " +
			   	"a.tftp_id="+tftp_id+" and (a.mon_time >= '"+starttime +"' and  a.mon_time <= '"+endtime+"') order by id";;
		   }else if("oracle".equalsIgnoreCase(SystemConstant.DBType)){
			   sql = "select a.is_canconnected,a.reason,a.mon_time from nms_tftp_history a where " +
			   	"a.tftp_id="+tftp_id+" and (a.mon_time >= "+"to_date('"+starttime+"','YYYY-MM-DD HH24:MI:SS')"+" and  a.mon_time <= "+"to_date('"+endtime+"','YYYY-MM-DD HH24:MI:SS')"+") order by id";;
		   }
//		   SysLogger.info(sql);
		   int i = 0;
		   double curPing=0;
