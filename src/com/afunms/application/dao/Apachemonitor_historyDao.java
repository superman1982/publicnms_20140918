...
public class Apachemonitor_historyDao extends BaseDao implements DaoInterface{
	SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
	   ...
	   public boolean save(BaseVo baseVo)
	   {
		   ...
		   //System.out.println("缺失逗号的一场SQL语句：========"+sql.toString());
		...
		*/
		...
		public Vector getByApacheid(Integer apacheid,String starttime,String totime,Integer isconnected) throws Exception{
	    	...
	    	try{
	    		String sql="";
	    		if("mysql".equalsIgnoreCase(SystemConstant.DBType)){
	    			sql="select a.is_canconnected,a.reason,a.mon_time from nms_apache_history a where " +
					"a.apache_id="+apacheid+" and (a.mon_time >= '"+starttime +"' and  a.mon_time <= '"+totime+"')";
	    		}else if("oracle".equalsIgnoreCase(SystemConstant.DBType)){
	    			sql="select a.is_canconnected,a.reason,a.mon_time from nms_apache_history a where " +
					"a.apache_id="+apacheid+" and (a.mon_time >= to_date('"+starttime+"','YYYY-MM-DD HH24:MI:SS') and  a.mon_time <= to_date('"+totime+"','YYYY-MM-DD HH24:MI:SS'))";
	    		}	    		
				//SysLogger.info(sql);
				rs = conn.executeQuery(sql);
				while(rs.next()){
	   ...
	   public BaseVo loadFromRS(ResultSet rs)
	   {
	       ...
	       {
	   	       SysLogger.error("Apachemonitor_historyDao.loadFromRS()",e); 
	       }	   
	       return vo;
	   }
