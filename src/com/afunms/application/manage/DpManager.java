...
public class DpManager extends BaseManager implements ManagerInterface {
	private String list() {
		...
		DpConfigDao configdao = new DpConfigDao();
		List list = null;
		try {
			if (operator.getRole() == 0) {
				list = configdao.loadAll();
			} else {
				list = configdao.getDpByBID(rbids);
			}
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
		...
		setTarget("/application/dp/dpconfiglist.jsp");
		return "/application/dp/dpconfiglist.jsp";
	}
	...
	private String add() {
		...
		try {
			...
			timeShareConfigUtil.saveTimeShareConfigList(request, String.valueOf(vo.getId()), timeShareConfigUtil.getObjectType("20"));
			TimeGratherConfigUtil timeGratherConfigUtil = new TimeGratherConfigUtil();
			timeGratherConfigUtil.saveTimeGratherConfigList(request, String.valueOf(vo.getId()), timeGratherConfigUtil.getObjectType("26"));

			// 初始化采集指标
			try {
				NodeGatherIndicatorsUtil nodeGatherIndicatorsUtil = new NodeGatherIndicatorsUtil();
				nodeGatherIndicatorsUtil.addGatherIndicatorsForNode(vo.getId() + "", "service", "dp", "1");
			...
			try {
				...
				alarmIndicatorsUtil.saveAlarmInicatorsThresholdForNode(String.valueOf(vo.getId()), "service", "dp");
			} catch (RuntimeException e) {
				e.printStackTrace();
			}
		...
		if (ShareData.getAlldps() != null) {
			ShareData.getAlldps().put(vo.getId(), vo);
		}
		...
		dpdao.clearRubbish(list);
		return "/dp.do?action=list&jp=1&flag=" + _flag;
	}
		...
		if (ids != null && ids.length > 0) {
			...
			try {
				...
				for (int i = 0; i < ids.length; i++) {
					vo = (DpConfig) configdao.findByID(ids[i]);
					tg.deleteTimeGratherConfig(ids[i], timeShareConfigUtil.getObjectType("20"));
					timeShareConfigUtil.deleteTimeShareConfig(ids[i], timeShareConfigUtil.getObjectType("20"));
					PollingEngine.getInstance().deleteDpByID(Integer.parseInt(ids[i]));
					String id = ids[i];
					...
					String sql = "delete from nms_hpdp where ipaddress = '" + vo.getIpAddress() + "'";
					try {
					...
					List dplist = nodedependao.findByNode("dp" + id);
					if (dplist != null && dplist.size() > 0) {
						for (int j = 0; j < dplist.size(); j++) {
							NodeDepend wesvo = (NodeDepend) dplist.get(j);
							if (wesvo != null) {
								LineDao lineDao = new LineDao();
								lineDao.deleteByidXml("dp" + id, wesvo.getXmlfile());
								NodeDependDao nodeDependDao = new NodeDependDao();
								if (nodeDependDao.isNodeExist("dp" + id, wesvo.getXmlfile())) {
									nodeDependDao.deleteByIdXml("dp" + id, wesvo.getXmlfile());
								} else {
									nodeDependDao.close();
								}
			...
			try {
				dao.deleteByNodeIdsAndTypeAndSubtype(ids, "service", "dp");
			} catch (RuntimeException e) {
				e.printStackTrace();
			} finally {
				dao.close();
			...
			try {
				indidao.deleteByNodeIds(ids, "service", "dp");
			} catch (RuntimeException e) {
				e.printStackTrace();
			} finally {
				indidao.close();
			...
			DpConfigDao dpdao = new DpConfigDao();
			List dpblist = new ArrayList();
			try {
				dpblist = dpdao.loadAll();
			} catch (Exception e) {
			...
			if (dpblist == null)
				dpblist = new ArrayList();
			ShareData.setDpconfiglist(dpblist);
			dpdao.clearRubbish(dpblist);
		}
		return "/dp.do?action=list&jp=1&flag=" + _flag;
	}
	...
	private String ready_edit() {
		String jsp = "/application/dp/edit.jsp";
		DpConfigDao dao = new DpConfigDao();
		try {
			setTarget(jsp);
			TimeShareConfigUtil timeShareConfigUtil = new TimeShareConfigUtil();
			List timeShareConfigList = timeShareConfigUtil.getTimeShareConfigList(getParaValue("id"), timeShareConfigUtil.getObjectType("20"));
			...
			// 提供已设置的采集时间信息
			TimeGratherConfigUtil tg = new TimeGratherConfigUtil();
			List<TimeGratherConfig> timeGratherConfigList = tg.getTimeGratherConfig(getParaValue("id"), tg.getObjectType("26"));
	...
	private String update() {
		...
		try {
			...
			Dp dp = (Dp) PollingEngine.getInstance().getDpByID(vo.getId());
	...
	private String addalert() {
		...
		List list = new ArrayList();
		DpConfigDao configdao = null;
		try {
			configdao = new DpConfigDao();
			vo = (DpConfig) configdao.findByID(getParaValue("id"));
			vo.setFlag(1);
			configdao.update(vo);
	...
	private String cancelalert() {
		...
		try {
			...
			// Web web = (Web)
			// PollingEngine.getInstance().getWebByID(vo.getId());
			// web.setFlag(0);
			DpLoader loader = new DpLoader();
			loader.loading();
		} catch (Exception e) {
	...
	private String detail() {
		...
		ResultSet rs = null;
		Hashtable hpdphash = new Hashtable();
		String sql = "select * from nms_hpdp where ipaddress = '" + vo.getIpAddress() + "' order by sessionId desc";
		...
		try {
			...
			while (rs.next()) {
				b++;
				hpdphash = new Hashtable();
				hpdphash.put("ipaddress", rs.getString("ipaddress"));
				hpdphash.put("sessionId", rs.getString("sessionId"));
				hpdphash.put("type", rs.getString("type"));
				hpdphash.put("status", rs.getString("status"));
				hpdphash.put("userGroup", rs.getString("userGroup"));
				hpdphash.put("collecttime", rs.getString("collecttime"));
				if (b == 1) {
	...
	private String dpevent() {
		...
		ResultSet rs = null;
		Hashtable hpdphash = new Hashtable();
		String sql = "select * from nms_hpdp where ipaddress = '" + vo.getIpAddress() + "'";
		Hashtable retValue = new Hashtable();
		Vector v = new Vector();
		String collecttime = "";
		DBManager dbmanager = new DBManager();
		...
		try {
			...
			try {
				// list =
				// eventdao.getQuery(starttime1,totime1,"db",status+"",level1+"",user.getBusinessids(),vo.getId());
				list = eventdao.getQuery(starttime1, totime1, "dp", "-1", "3", user.getBusinessids(), vo.getId());
			} catch (Exception e) {
				e.printStackTrace();
			} finally {
				eventdao.close();
		...
		request.setAttribute("vo", vo);
		request.setAttribute("returnHash", returnHash);
		return "/application/dp/dpevent.jsp";
	}
