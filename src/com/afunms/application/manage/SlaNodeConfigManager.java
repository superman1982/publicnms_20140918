...
public class SlaNodeConfigManager extends BaseManager implements ManagerInterface
{
	...
	private String listperf()
	{
		...
		request.setAttribute("list",list);
		return "/application/sla/listperf.jsp";
	}
...
//	}
    ...
    {    	   
		...
		String entrynumber = getParaValue("entrynumber");
		String  adminsign=getParaValue("adminsign");
		String operatesign=getParaValue("operatesign");
		String bid = getParaValue("bid");
		SlaNodeConfig slanode = new SlaNodeConfig();
		List allslalist = new ArrayList();
		String[] intervals = poll_interval.split("-");
				...
				try{
					for(int i=0;i<ips.length;i++){
						...
						if(_ip != null && _ip.trim().length()>0){
							...
							slanode.setMon_flag(mon_flag);
							slanode.setSlatype(slatype);
							slanode.setDevicetype(devicetype);
							slanode.setCollecttype(collecttype);
							slanode.setAdminsign(adminsign);
							slanode.setOperatesign(operatesign);
							allslalist.add(slanode);
						}
					}
				}catch(Exception e){
		...
		if(allslalist != null && allslalist.size()>0){
			...
			try{
				sladao.save(allslalist);
				for(int i=0;i<allslalist.size();i++){
					SlaNodeConfig _slanode = (SlaNodeConfig)allslalist.get(i);
					if(_slanode.getSlatype().contains("tcpconnect"))_slanode.setSlatype("tcp");
					AlarmIndicatorsUtil alarmIndicatorsUtil = new AlarmIndicatorsUtil();
   					alarmIndicatorsUtil.saveAlarmInicatorsThresholdForNode(_slanode.getId()+"", "sla",_slanode.getSlatype());
				}
			}catch(Exception e){
				e.printStackTrace();
			}
...
//			String totime1 = t_time + " 23:59:59";
			...
			try{
				...
				eventlist = eventdao.getQuery(starttime1,totime1,"ciscosla",status+"",level1+"",
