...
public class PortTrafficReport extends ImplementorReport
{
	private int nodeId;
    ...
    {
    	head = "端口流量报表";
    	tableHead = new String[]{"序号","时间","入口流量","出口流量"};   	
    	colWidth = new int[]{1,2,2,2};
    	
    	NodeDataDao dao = new NodeDataDao();
    	String[][] result = dao.portTrafficStat(nodeId, timeStamp, ifIndex);
		...
		TimeSeries outSeries = new TimeSeries("出口流量",Hour.class);
		TimeSeries inSeries = new TimeSeries("入口流量",Hour.class);
		...
		chartKey = ChartCreator.createTimeSeriesChart(datasetOut,datasetIn,"X-时间(h)","Y-流量(kbps)","出口/入口流量",500,350);
