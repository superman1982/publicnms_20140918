...
//	}
				...
				// 生成线程池
		  		...
		  		try{
			  		...
			  		for (int i=0; i<ipMacList.size(); i++) {
			      			threadPool.runTask(createTask((IpMac)ipMacList.get(i) , ipDistrictList,macHash,disHash));
			  		}
	...
	public Runnable createTask(final IpMac ipMac , final List ipDistrictList,final Hashtable macHash,final Hashtable disHash){
		return new Runnable(){
			...
			public void run() {
				...
				DistrictConfig originalDistrict = getOriginalDistrict(mac,macHash,disHash);
