...
public class BaseTelnet {
	 ...
	 * 带参数构造方法
	 */
	public BaseTelnet(String ip, String user, String password, int port, String suuser, String supassword) {
		this.ip = ip;
		this.port = port;
		this.user = user; // 默认用户
		...
		try {
			...
			if (connetflg) {// 成功
				if (null != this.password && this.password.trim().length() > 0) {
					...
					if (aa.indexOf("Login failed!") > 0 || aa.indexOf("user or password error") > 0||aa.endsWith(":")) {
						isLogin = false;
						msg = "用户登录失败";
	...
	public String readUntil(String pattern1, String pattern2) {
		...
		try {
			...
			while (flag) {
				...
				if (sb.toString().indexOf("--More--") > 0 || sb.toString().toLowerCase().indexOf("---- more ----") > 0) {
					out.write(32);
					out.flush();
				}
				ch = (char) in.read();
