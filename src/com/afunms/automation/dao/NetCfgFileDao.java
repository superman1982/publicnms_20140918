...
//		System.out.println(sql);
	...
	public BaseVo loadFromRS(ResultSet rs) {
		...
		try {
			...
			SysLogger.error("NetCfgFileDao.loadFromRS()", e);
			vo = null;
		}
		return vo;
	}
	...
	public List getDeviceByIps(List iplist,String type) {
		...
		List list = null;
		String sql = "select max(id) as id,ipaddress,fileName,descri,max(backup_time) as backup_time,file_size,bkp_type,baseline from nms_hua3vpncfg where ipaddress in('"+sBuffer.toString()+") and bkp_type='"+type+"' group by ipaddress,bkp_type";
		list = this.findByCriteria(sql);
		return list;
	}
