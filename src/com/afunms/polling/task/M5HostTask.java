...
public class M5HostTask extends MonitorTask {
...
//		    		}
	    	...
	    	if(monitorItemList == null)monitorItemList = new ArrayList<NodeGatherIndicators>();
        		...
        		if(docollcetHash != null && docollcetHash.size()>0){
            		...
            		Hashtable allsystemdataHash = new Hashtable();
            		...
            		Hashtable arpHash = new Hashtable();
            		ParserXmlUtil xmlutil = new ParserXmlUtil();
            		try{
            			File file=null;
            			String filename=null;
            			//连通率
                			filename = ResourceCenter.getInstance().getSysPath() + "linuxserver/host_ping.xml";				
                			 file=new File(filename);
                			if(file.exists()){
                				try{
            			          pingHash=xmlutil.parserNetWorkXml(filename,"ping");
                			...
                			//系统组信息
                		  filename = ResourceCenter.getInstance().getSysPath()+ "linuxserver/host_systemgroup.xml";				
               			  file=new File(filename);
               			 if(file.exists()){
               				try{
                			allsystemdataHash = xmlutil.parserXml(filename);
                    		dataToMem(allsystemdataHash, alldata, "system", "systemgroup");
               				}catch(Exception e){
                			...
                			//cpu
                			 filename =ResourceCenter.getInstance().getSysPath()+ "linuxserver/host_cpu.xml";				
                			 file=new File(filename);
                			if(file.exists()){
                				try{
            			           cpuHash = xmlutil.parserNetWorkXml(filename,"cpu");
                			...
                			//内存
                		  filename = ResourceCenter.getInstance().getSysPath()+ "linuxserver/host_mem.xml";				
               			 file=new File(filename);
            			 if(file.exists()){
            				try{
            			       memHash = xmlutil.parserNetWorkXml(filename,"memory");
            		...
            		//	filename = ResourceCenter.getInstance().getSysPath()+ "linuxserver/host_mem.xml";				
            			...
            			if(file.exists()){
            				try{
            			        phymemHash = xmlutil.parserNetWorkXml(filename,"PhysicalMemory");
            			...
            			if(file.exists()){
            				try{
            			       virmemHash = xmlutil.parserNetWorkXml(filename,"VirtualMemory");
            			...
            			//磁盘
            			 filename = ResourceCenter.getInstance().getSysPath()+ "linuxserver/host_disk.xml";				
            			 file=new File(filename);
            			if(file.exists()){
            				try{
            			         diskHash = xmlutil.parserNetWorkXml(filename,"disk");
            			...
            			//arp
            			 filename = ResourceCenter.getInstance().getSysPath()+"linuxserver/host_arp.xml";				
            			 file=new File(filename);
            			if(file.exists()){
            				try{
            			        arpHash=xmlutil.parserFdbXml(filename);
            			        dataToMem(arpHash, alldata, "ipmac", "ipmac");
                             }catch(Exception e){
...
//        		}
			...
			SysLogger.info("********M5HostTask Thread Count : "+Thread.activeCount());
		}
	}
	/**
	 * @author wxy add
            ...
            try {                	
            	...
            	if(host.getCollecttype() == SystemConstant.COLLECTTYPE_DATAINTERFACE){
            		...
            		if(interfacedataHash != null && interfacedataHash.size()>0){
        			   ...
        			   // ShareData.getSharedata().put("interface"+host.getIpAddress(), interfacedataHash);//wxy edit
            		}
            	return;
            	}   
