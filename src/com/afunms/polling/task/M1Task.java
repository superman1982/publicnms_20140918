 ...
 *
 */
public class M1Task extends MonitorTask {
...
//			ShareData.setPortConfigHash(portconfigHash);
	    	...
	    	try{
	    		//获取被启用的所有被监视指标
	    		monitorItemList = indicatorsdao.getByIntervalAndType("1", "m",1,"net");
	    	}catch(Exception e){
	    		e.printStackTrace();
	    	}finally{
	    		indicatorsdao.close();
...
//        			threadPool.runTask(createTask(nodeGatherIndicators,alldata));           		
        		...
        		if(docollcetHash != null && docollcetHash.size()>0){
        			...
        			while(newProEnu.hasMoreElements())
        			{
        				...
        				dolist = (List)docollcetHash.get(nodeid);
			...
			SysLogger.info("********M1Task Thread Count : "+Thread.activeCount());
		}
	}
