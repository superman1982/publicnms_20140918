...
import com.afunms.polling.snmp.cpu.MotorolaCpuSnmp;
...
import com.afunms.polling.snmp.fan.BrocadeFanSnmp;
import com.afunms.polling.snmp.fan.CiscoFanSnmp;
import com.afunms.polling.snmp.fan.H3CFanSnmp;
import com.afunms.polling.snmp.fan.MotorolaFanSnmp;
...
import com.afunms.polling.snmp.flash.MotorolaFlashSnmp;
import com.afunms.polling.snmp.interfaces.ArpSnmp;
import com.afunms.polling.snmp.interfaces.BrocadeInterfaceSnmp;
...
import com.afunms.polling.snmp.memory.MotorolaMemorySnmp;
...
import com.afunms.polling.snmp.temperature.MotorolaTemperatureSnmp;
import com.afunms.polling.snmp.voltage.CiscoVoltageSnmp;
import com.afunms.polling.snmp.voltage.H3CVoltageSnmp;
import com.afunms.polling.snmp.voltage.MotorolaVoltageSnmp;
 ...
 *
 */
public class M5Task extends MonitorTask {
	
	public void run() {
		try{
			//每次重新调度task时，刷新其日期属性
			super.recentlyStartTime = new Date();
			//SysLogger.info("----启动task-"+this.getClass().getName()+" 时间： "+super.sdf.format(recentlyStartTime));
			//SysLogger.info("wwwwwwwwwwwwwwwwwwwwwwM5Taskwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww: "+Thread.activeCount());
			...
			if(portconfiglist != null && portconfiglist.size()>0){
				for(int i=0;i<portconfiglist.size();i++){
					...
					if(portconfigHash.containsKey(portconfig.getIpaddress())){
						...
						SysLogger.info("################################");
						SysLogger.info("add "+portconfig.getIpaddress()+"====size:"+portlist.size());
						SysLogger.info("################################");
					}else{
...
//				 try{
//					 pollmg.executeQuery("select * from nms_email_history");
//				 }catch(Exception e){
//					 
//				 }finally{
//					 //pollmg.close(); 
...
//			}
	...
	private void dataToMem(Hashtable allsystemdataHash,Hashtable alldata,String ipAllDataName,String ipdataName){	
		...
		while(newProEnu.hasMoreElements())
		{
			...
			if (host.getCollecttype() == SystemConstant.COLLECTTYPE_DATAINTERFACE) {//传输方向为南向
...
//					Hashtable ipdata1=(Hashtable)alldata.get(nodeip);
//					Hashtable dataHash1=(Hashtable)ipdata1.get(ipdataName);
//					Vector vector=(Vector)dataHash1.get(ipAllDataName);
//					System.out.println("////"+vector);
	...
	private static void ipDataToMem(Hashtable ipdataHash,Hashtable alldata,String ip,String ipDataName){
		if(ipdataHash != null && ipdataHash.size()>0){
			if(alldata.containsKey(ip)){
				Hashtable ipdata = (Hashtable)alldata.get(ip);
				if(ipdata != null){
					ipdata.put(ipDataName, ipdataHash);
				}else{
					ipdata = new Hashtable();
					ipdata.put(ipDataName, ipdataHash);
				}
				alldata.put(ip, ipdata);
			...
			//关于流速放入内存的key(修改为interface+ip),这样可以和cpu、内存等等指标区分开，再次轮询不再覆盖其他指标，减少不必要的麻烦
		   // ShareData.getSharedata().put(ipDataName+ip, ipdataHash);//wxy update
		}
	}
	/**
    创建任务
    ...
    return new Runnable() {
        public void run() {
        	try {                	
            	...
            	if(node.getCollecttype() == SystemConstant.COLLECTTYPE_DATAINTERFACE){
            		//接口数据方式进行采集,当前主要采集系统信息、接口信息
            		//进行文件解析
            		Hashtable interfacedataHash = new Hashtable();
            		Hashtable arpDataHash=new Hashtable();
            		ParserXmlUtil xmlutil = new ParserXmlUtil();
            		
            		try{
            			SysLogger.info("开始解析"+ResourceCenter.getInstance().getSysPath()+ "linuxserver/net_"+node.getIpAddress()+".xml");
            			String filename = ResourceCenter.getInstance().getSysPath() + "linuxserver/net_"+node.getIpAddress()+".xml";				
            			File file=new File(filename);
            			if(file.exists()){
            				try{
            			interfacedataHash = xmlutil.parserInterfaceXml(ResourceCenter.getInstance().getSysPath()+ "linuxserver/net_"+node.getIpAddress()+".xml",node.getIpAddress());
            			ipDataToMem(interfacedataHash,alldata,node.getIpAddress(),"interface");

            			try
            			{
            				File delFile = new File(ResourceCenter.getInstance().getSysPath() + "linuxserver/net_"+node.getIpAddress()+".xml");
            				//delFile.delete();
            			}catch(Exception e)	{
            				e.printStackTrace();
            			}
            			...
            			//解析arp信息
            			 filename = ResourceCenter.getInstance().getSysPath() + "linuxserver/net_arp"+node.getIpAddress()+".xml";				
            			 file=new File(filename);
            			if(file.exists()){
            		try{
            			arpDataHash=xmlutil.parserArpXml(ResourceCenter.getInstance().getSysPath()+ "linuxserver/net_arp"+node.getIpAddress()+".xml");
            			ipDataToMem(arpDataHash,alldata,node.getIpAddress(),"ipmac");
            			try{
            				File delFile = new File(ResourceCenter.getInstance().getSysPath() + "linuxserver/net_arp"+node.getIpAddress()+".xml");
            			   //	delFile.delete();
            			}catch(Exception e)	{
            				e.printStackTrace();
            			}
            	...
            	if(list != null && list.size()>0){
            		for(int k=0;k<list.size();k++){
            			//if(node.getCollecttype() != SystemConstant.COLLECTTYPE_SNMP)return;
            			...
            			if(alarmIndicatorsNode.getName().equalsIgnoreCase("cpu")){
                    		...
                    		}else if(alarmIndicatorsNode.getSubtype().equalsIgnoreCase("motorola")){
                    			//motorola的CPU
                    			//Host node = (Host)PollingEngine.getInstance().getNodeByID(Integer.parseInt(alarmIndicatorsNode.getNodeid()));
                    			if(node != null){
                    				MotorolaCpuSnmp motorolaCpuSnmp = null;
                        			try{
                        				motorolaCpuSnmp = (MotorolaCpuSnmp)Class.forName("com.afunms.polling.snmp.cpu.MotorolaCpuSnmp").newInstance();
                            			returnHash = motorolaCpuSnmp.collect_Data(alarmIndicatorsNode);
                            			if(alldata.containsKey(node.getIpAddress())){
                            				Hashtable ipdata = (Hashtable)alldata.get(node.getIpAddress());
                            				if(ipdata != null){
                            					ipdata.put("cpu", returnHash);
                    		...
                    		if(alarmIndicatorsNode.getSubtype().equalsIgnoreCase("cisco")){
                    			...
                    			//Host node = (Host)PollingEngine.getInstance().getNodeByID(Integer.parseInt(alarmIndicatorsNode.getNodeid()));
                    			if(node != null){
                    				MotorolaMemorySnmp motorolamemorysnmp = null;
                        			try{
                        				Date startdate1 = new Date();
                        				motorolamemorysnmp = (MotorolaMemorySnmp)Class.forName("com.afunms.polling.snmp.memory.MotorolaMemorySnmp").newInstance();
                            			returnHash = motorolamemorysnmp.collect_Data(alarmIndicatorsNode);
                            			Date enddate1 = new Date();
                                		SysLogger.info("##############################");
                        				SysLogger.info("### "+node.getIpAddress()+" 网络设备Memory采集时间 "+(enddate1.getTime()-startdate1.getTime())+"####");
                        				SysLogger.info("##############################");
                    		...
                    		if(node != null){
                    			...
                    			BrocadeInterfaceSnmp brocadeInterfaceSnmp = null;
                    			try{
//                    				SysLogger.info("############################################################");
//                    				SysLogger.info("##########开始采集"+node.getIpAddress()+" 接口信息...##########");
//                    				SysLogger.info("############################################################");
//                    				Date startdate1 = new Date();
                    				if(alarmIndicatorsNode.getSubtype().equalsIgnoreCase("brocade")){//博科的网络设备interface采集
                    					brocadeInterfaceSnmp = (BrocadeInterfaceSnmp)Class.forName("com.afunms.polling.snmp.interfaces.BrocadeInterfaceSnmp").newInstance();
                    					returnHash = brocadeInterfaceSnmp.collect_Data(alarmIndicatorsNode);
                    				}else{
...
//                    				SysLogger.info("##############################");
                    		...
                    		if(node != null){
                    			if(alarmIndicatorsNode.getSubtype().equalsIgnoreCase("cisco")){
                    				...
                    				//motorola闪存
                    				MotorolaFlashSnmp motorolaFlashSnmp = null;
                        			try{
                        				//SysLogger.info("开始采集h3c的FLASH信息################");
                        				Date startdate1 = new Date();
                        				motorolaFlashSnmp = (MotorolaFlashSnmp)Class.forName("com.afunms.polling.snmp.flash.MotorolaFlashSnmp").newInstance();
                            			returnHash = motorolaFlashSnmp.collect_Data(alarmIndicatorsNode);
                            			Date enddate1 = new Date();
                                 		SysLogger.info("##############################");
                         				SysLogger.info("### "+node.getIpAddress()+" 网络设备FLASH采集时间 "+(enddate1.getTime()-startdate1.getTime())+"####");
                         				SysLogger.info("##############################");
                    		...
                    		if(node != null){
                    			if(alarmIndicatorsNode.getSubtype().equalsIgnoreCase("cisco")){
                    			...
                    			}else if(alarmIndicatorsNode.getSubtype().equalsIgnoreCase("brocade")){
                    				//博科温度
                    				BrocadeTemperatureSnmp tempersnmp = null;
                        			try{
                        				//SysLogger.info("开始采集BDCOM温度信息################");
                        				tempersnmp = (BrocadeTemperatureSnmp)Class.forName("com.afunms.polling.snmp.temperature.BrocadeTemperatureSnmp").newInstance();
                            			returnHash = tempersnmp.collect_Data(alarmIndicatorsNode);
                            			//FDB表不存入数据库
                            			if(alldata.containsKey(node.getIpAddress())){
                            				Hashtable ipdata = (Hashtable)alldata.get(node.getIpAddress());
                    				...
                    				//motorola温度
                    				MotorolaTemperatureSnmp motorolaTemperatureSnmp = null;
                        			try{
                        				//SysLogger.info("开始采集H3C温度信息################");
                        				Date startdate1 = new Date();
                        				motorolaTemperatureSnmp = (MotorolaTemperatureSnmp)Class.forName("com.afunms.polling.snmp.temperature.MotorolaTemperatureSnmp").newInstance();
                            			returnHash = motorolaTemperatureSnmp.collect_Data(alarmIndicatorsNode);
                            			Date enddate1 = new Date();
                                 		SysLogger.info("##############################");
                         				SysLogger.info("### "+node.getIpAddress()+" 网络设备温度采集时间 "+(enddate1.getTime()-startdate1.getTime())+"####");
                         				SysLogger.info("##############################");
                    		...
                    		if(node != null){
                    			if(alarmIndicatorsNode.getSubtype().equalsIgnoreCase("cisco")){
                    				...
                    				//博科风扇
                    				BrocadeFanSnmp fansnmp = null;
                        			try{
                        				//SysLogger.info("开始采集温度信息################");
                        				fansnmp = (BrocadeFanSnmp)Class.forName("com.afunms.polling.snmp.fan.BrocadeFanSnmp").newInstance();
                            			returnHash = fansnmp.collect_Data(alarmIndicatorsNode);
                            			//FDB表不存入数据库
                            			if(alldata.containsKey(node.getIpAddress())){
                            				Hashtable ipdata = (Hashtable)alldata.get(node.getIpAddress());
                    				...
                    				//motorola风扇
                    				MotorolaFanSnmp motorolaFanSnmp = null;
                        			try{
                        				//SysLogger.info("开始采集温度信息################");
                        				Date startdate1 = new Date();
                        				motorolaFanSnmp = (MotorolaFanSnmp)Class.forName("com.afunms.polling.snmp.fan.MotorolaFanSnmp").newInstance();
                            			returnHash = motorolaFanSnmp.collect_Data(alarmIndicatorsNode);
                            			Date enddate1 = new Date();
                                 		SysLogger.info("##############################");
                         				SysLogger.info("### "+node.getIpAddress()+" 网络设备FAN采集时间 "+(enddate1.getTime()-startdate1.getTime())+"####");
                         				SysLogger.info("##############################");
                    		...
                    		if(node != null){
                    			if(alarmIndicatorsNode.getSubtype().equalsIgnoreCase("cisco")){
                    				...
                    				//brocade电源
                    				BrocadePowerSnmp powersnmp = null;
                        			try{
                        				//SysLogger.info("开始采集CISCO电源信息################");
                        				powersnmp = (BrocadePowerSnmp)Class.forName("com.afunms.polling.snmp.power.BrocadePowerSnmp").newInstance();
                            			returnHash = powersnmp.collect_Data(alarmIndicatorsNode);
                            			//存入数据库
                            			if(alldata.containsKey(node.getIpAddress())){
                            				Hashtable ipdata = (Hashtable)alldata.get(node.getIpAddress());
                    				...
                    				//motorola电源
                    				MotorolaPowerSnmp motorolaPowerSnmp = null;
                        			try{
                        				//SysLogger.info("开始采集H3C电源信息################");
                        				Date startdate1 = new Date();
                        				motorolaPowerSnmp = (MotorolaPowerSnmp)Class.forName("com.afunms.polling.snmp.power.MotorolaPowerSnmp").newInstance();
                            			returnHash = motorolaPowerSnmp.collect_Data(alarmIndicatorsNode);
                            			Date enddate1 = new Date();
                                 		SysLogger.info("##############################");
                         				SysLogger.info("### "+node.getIpAddress()+" 网络设备POWER采集时间 "+(enddate1.getTime()-startdate1.getTime())+"####");
                         				SysLogger.info("##############################");
                    		...
                    		if(node != null){
                    			if(alarmIndicatorsNode.getSubtype().equalsIgnoreCase("cisco")){
                    				...
                    				//motorola电压
                    				MotorolaVoltageSnmp voltagesnmp = null;
                        			try{
                        				//SysLogger.info("开始采集H3C电压信息################");
                        				Date startdate1 = new Date();
                        				voltagesnmp = (MotorolaVoltageSnmp)Class.forName("com.afunms.polling.snmp.voltage.MotorolaVoltageSnmp").newInstance();
                            			returnHash = voltagesnmp.collect_Data(alarmIndicatorsNode);
                            			Date enddate1 = new Date();
                                 		SysLogger.info("##############################");
                         				SysLogger.info("### "+node.getIpAddress()+" 网络设备电压采集时间 "+(enddate1.getTime()-startdate1.getTime())+"####");
