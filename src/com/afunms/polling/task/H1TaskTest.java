 ...
 *
 */
public class H1TaskTest extends MonitorTask {
	...
	public void run(){
    	...
    	try {                	
        	...
        	//取出被监视节点的所有指标
        	//List gatherIndicatorsList = TaskUtil.getGatherIndicatorsList(nodeid, "1", "h", 1, "net");
        	List gatherIndicatorsList = (List)ShareData.getGatherHash().get(nodeid+":net:1:h");
...
//	        			Host node = (Host)PollingEngine.getInstance().getNodeByID(Integer.parseInt(alarmIndicatorsNode.getNodeid()));
	        			if(node != null){
	            			try{
	                			...
	                			if(ShareData.getH1Alldata().containsKey(node.getIpAddress())){
	                				Hashtable ipdata = (Hashtable)ShareData.getH1Alldata().get(node.getIpAddress());
	                				if(ipdata != null){
	                				...
	                				ShareData.getH1Alldata().put(node.getIpAddress(), ipdata);
	                			}else{
...
//	            				if(ShareData.getH1Alldata().get(node.getIpAddress()) != null){
//	            					((Hashtable)ShareData.getH1Alldata().get(node.getIpAddress())).put("interface", returnHash);
//	            				}else{
//	            					Hashtable ipdata = new Hashtable();
//	            					ipdata.put("interface", returnHash);
//	            					ShareData.getH1Alldata().put(node.getIpAddress(), ipdata);
...
//	            			}
        	...
        	int h1CollectedSize = -1;
        	if(!nodeid.equals("")){
        		h1CollectedSize = ShareData.addH1CollectedSize();
        	}
        	int needCollectNodesSize = ShareData.getH1TimerMap().keySet().size();
        	//System.out.println("####nodeid:"+nodeid+"####needCollectNodesSize:"+needCollectNodesSize+"====ShareData.getM5CollectedSize():"+m5CollectedSize);
        	//判断所有Task是否运行完毕
			if(needCollectNodesSize == h1CollectedSize){//需要采集的设备个数 和 已采集的设备个数相等，则保存
	    		ShareData.setH1CollectedSize(0);
				Date _enddate = new Date();
	        	HostCollectDataManager hostdataManager=new HostCollectDataManager(); 
	    		Date startdate = new Date();
	    		try{
	    			hostdataManager.createHostItemData(ShareData.getH1Alldata(),"net");  
	    		}catch(Exception e){
	    		...
	    		SysLogger.info("##############################");
	    		ShareData.setH1Alldata(new Hashtable());//清空
//	    		alldata.clear();
//				alldata = null;
				//查询数据库配置 并把数据更新到内存中
				//updateConnectTypeConfig();
				SysLogger.info("********H1Task Thread Count : "+Thread.activeCount());
...
//					checkEventHashtable.put(vo.getName(), vo.getAlarmlevel());
	...
	public static Hashtable getDocollcetHash(){
		...
		try {
			// 获取被启用的所有被监视指标
			monitorItemList = indicatorsdao.getByIntervalAndType("1", "h", 1,"net");
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			indicatorsdao.close();
