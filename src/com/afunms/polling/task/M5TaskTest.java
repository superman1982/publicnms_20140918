 ...
 *
 */
public class M5TaskTest extends MonitorTask {
	...
	public void run(){
		...
		//Hashtable alldata = ShareData.getM5Alldata();
    	try {                	
        	...
        	//取出被监视节点的所有指标
        	//List gatherIndicatorsList = TaskUtil.getGatherIndicatorsList(nodeid, "5", "m", 1, "net");
        	List gatherIndicatorsList = (List)ShareData.getGatherHash().get(nodeid+":net:5:m");   
...
//	        			Host node = (Host)PollingEngine.getInstance().getNodeByID(Integer.parseInt(alarmIndicatorsNode.getNodeid()));
	        			if(node != null){
	            			try{
	                			...
	                			if(ShareData.getM5Alldata().containsKey(node.getIpAddress())){
	                				Hashtable ipdata = (Hashtable)ShareData.getM5Alldata().get(node.getIpAddress());
	                				if(ipdata != null){
	                				...
	                				ShareData.getM5Alldata().put(node.getIpAddress(), ipdata);
	                			}else{
...
//	            				if(ShareData.getM5Alldata().get(node.getIpAddress()) != null){
//	            					((Hashtable)ShareData.getM5Alldata().get(node.getIpAddress())).put("interface", returnHash);
//	            				}else{
//	            					Hashtable ipdata = new Hashtable();
//	            					ipdata.put("interface", returnHash);
//	            					ShareData.getM5Alldata().put(node.getIpAddress(), ipdata);
...
//	            			}
                    						...
                    						for(int m = 0 ; m < list.size() ; m ++){
                    							...
                    							if("1".equals(_alarmIndicatorsNode.getEnabled())){
                    								if(_alarmIndicatorsNode.getName().equalsIgnoreCase("ping")){
                    									...
                    									checkutil.checkEvent(node, _alarmIndicatorsNode, pingdata.getThevalue());
                    								}
                    							}  
                    						}
        	...
        	int m5CollectedSize = -1;
        	if(!nodeid.equals("")){
        		m5CollectedSize = ShareData.addM5CollectedSize();
        	}
        	int needCollectNodesSize = ShareData.getM5TimerMap().keySet().size();
        	//System.out.println("####nodeid:"+nodeid+"####needCollectNodesSize:"+needCollectNodesSize+"====ShareData.getM5CollectedSize():"+m5CollectedSize);
        	//判断所有Task是否运行完毕
			if(needCollectNodesSize == m5CollectedSize){//需要采集的设备个数 和 已采集的设备个数相等，则保存
	    		ShareData.setM5CollectedSize(0);
				Date _enddate = new Date();
	        	HostCollectDataManager hostdataManager=new HostCollectDataManager(); 
	    		Date startdate = new Date();
	    		try{
	    			hostdataManager.createHostItemData(ShareData.getM5Alldata(),"net");  
	    		}catch(Exception e){
	    		...
	    		SysLogger.info("##############################");
	    		ShareData.setM5Alldata(new Hashtable());//清空
//	    		alldata.clear();
//				alldata = null;
				//查询数据库配置 并把数据更新到内存中
...
//					checkEventHashtable.put(vo.getName(), vo.getAlarmlevel());
	...
	public static Hashtable getDocollcetHash(){
		...
		try {
			// 获取被启用的所有被监视指标
			monitorItemList = indicatorsdao.getByIntervalAndType("5", "m", 1,"net");
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			indicatorsdao.close();
