 ...
 *
 */
public class M10HostTaskTest extends MonitorTask {
	...
	public void run(){
    	...
    	try {                	
        	...
        	//取出被监视节点的所有指标
        	//List list = TaskUtil.getGatherIndicatorsList(nodeid, "10", "m", 1, "host");
        	List list = (List)ShareData.getGatherHash().get(nodeid+":host:10:m");
            			...
            			if(node != null){
            				if(node.isManaged()){
                    			...
                    			try{
                        			...
                        			windowscpusnmp = null;
                        			if(ShareData.getM10HostAlldata().containsKey(node.getIpAddress())){
    	                				Hashtable ipdata = (Hashtable)ShareData.getM10HostAlldata().get(node.getIpAddress());
    	                				if(ipdata != null){
    	                				...
    	                				ShareData.getM10HostAlldata().put(node.getIpAddress(), ipdata);
    	                			}else{
        	...
        	int m10HostCollectedSize = -1;
        	if(!nodeid.equals("")){
        		m10HostCollectedSize = ShareData.addM10HostCollectedSize();
        	}
        	int needCollectNodesSize = ShareData.getM10HostTimerMap().keySet().size();
        	//System.out.println("####nodeid:"+nodeid+"####needCollectNodesSize:"+needCollectNodesSize+"====ShareData.getM5CollectedSize():"+m5CollectedSize);
        	//判断所有Task是否运行完毕
			if(needCollectNodesSize == m10HostCollectedSize){//需要采集的设备个数 和 已采集的设备个数相等，则保存
	    		ShareData.setM10HostCollectedSize(0);
				Date _enddate = new Date();
	        	HostCollectDataManager hostdataManager=new HostCollectDataManager(); 
	    		Date startdate = new Date();
	    		try{
	    			hostdataManager.createHostItemData(ShareData.getM10HostAlldata(),"host");  
	    		}catch(Exception e){
	    		...
	    		SysLogger.info("##############################");
	    		ShareData.setM10HostAlldata(new Hashtable());//清空
				//查询数据库配置 并把数据更新到内存中
				//updateConnectTypeConfig();
				SysLogger.info("********M10HostTask Thread Count : "+Thread.activeCount());
...
//					checkEventHashtable.put(vo.getName(), vo.getAlarmlevel());
	...
	public static Hashtable getDocollcetHash(){
		...
		try {
			// 获取被启用的所有被监视指标
			monitorItemList = indicatorsdao.getByIntervalAndType("10", "m", 1,"host");
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			indicatorsdao.close();
