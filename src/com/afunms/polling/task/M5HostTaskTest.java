 ...
 *
 */
public class M5HostTaskTest extends MonitorTask {
	...
	public void run(){
    	...
    	try {                	
        	...
        	//取出被监视节点的所有指标
        	//List list = TaskUtil.getGatherIndicatorsList(nodeid, "5", "m", 1, "host");
        	List list = (List)ShareData.getGatherHash().get(nodeid+":host:5:m");
            			...
            			if(node != null){
            				if(node.isManaged()){
                    			...
                    			try{
                        			...
                        			windowscpusnmp = null;
                        			if(ShareData.getM5HostAlldata().containsKey(node.getIpAddress())){
    	                				Hashtable ipdata = (Hashtable)ShareData.getM5HostAlldata().get(node.getIpAddress());
    	                				if(ipdata != null){
    	                				...
    	                				ShareData.getM5HostAlldata().put(node.getIpAddress(), ipdata);
    	                			}else{
        	...
        	int m5HostCollectedSize = -1;
        	if(!nodeid.equals("")){
        		m5HostCollectedSize = ShareData.addM5HostCollectedSize();
        	}
        	int needCollectNodesSize = ShareData.getM5HostTimerMap().keySet().size();
        	//System.out.println("####nodeid:"+nodeid+"####needCollectNodesSize:"+needCollectNodesSize+"====ShareData.getM5CollectedSize():"+m5CollectedSize);
        	//判断所有Task是否运行完毕
			if(needCollectNodesSize == m5HostCollectedSize){//需要采集的设备个数 和 已采集的设备个数相等，则保存
	    		ShareData.setM5HostCollectedSize(0);
				Date _enddate = new Date();
	        	HostCollectDataManager hostdataManager=new HostCollectDataManager(); 
	    		Date startdate = new Date();
	    		try{
	    			hostdataManager.createHostItemData(ShareData.getM5HostAlldata(),"host");  
	    		}catch(Exception e){
	    		...
	    		SysLogger.info("##############################");
	    		ShareData.setM5HostAlldata(new Hashtable());//清空
				//查询数据库配置 并把数据更新到内存中
