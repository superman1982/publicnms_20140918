 ...
 */
public class CiscoDhcpScopeSnmp extends AbstractSnmp {
	 ...
	 * @param timeout
	 */
	public CiscoDhcpScopeSnmp(String host,String community,Integer port) {
	...
	public  Hashtable collectData(){
		...
		List dhcpscopeValue = collectCISCODHCPScopeData();
	...
	public  Hashtable collectData(Hashtable gatherhash,DHCPConfig dhcpconf){
		...
		if(gatherhash.containsKey("dhcppool")){
			//采集域
			try{
				dhcpscopeValue = collectCISCODHCPScopeData();
			}catch(Exception e){
...
//		}		
	...
	public  List collectCISCODHCPScopeData(){
		...
		try {
					try{					
							  String[] oids =                
								  new String[] {  
									".1.3.6.1.4.1.9.10.102.1.4.1.1.1", 	//NetName
									".1.3.6.1.4.1.9.10.102.1.4.1.1.4",  //Free
								    ".1.3.6.1.4.1.9.10.102.1.4.1.1.5", //Reserved
									".1.3.6.1.4.1.9.10.102.1.4.1.1.6" //Total
				...
				if (vb != null){
					...
					for(int j=0;j<vb.length;j++){
						if(vb[j]!=null){
							...
							if(j==0){
								scope.put("netname", sValue);
								SysLogger.info("==========netname:"+sValue);
							}else if (j==1){
								scope.put("free", sValue);
								SysLogger.info("==========free:"+sValue);
							}else if (j==2){
								scope.put("reserved", sValue);
								SysLogger.info("==========reserved:"+sValue);
							}else if (j==3){
								scope.put("total", sValue);
								SysLogger.info("==========total:"+sValue);
