package com.afunms.polling.snmp.cache;
...
import com.gatherResulttosql.HDSRunCableResultTosql;
 ...
 */
public class HDCRunCableSnmp extends SnmpMonitor {
	//private Hashtable sendeddata = ShareData.getProcsendeddata();
	java.text.SimpleDateFormat sdf = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
	/**
	 * 
	 */
	public HDCRunCableSnmp() {
	}
		   		  ...
		   		  //if(node.getSysOid().startsWith("1.3.6.1.4.1.2011.") || node.getSysOid().startsWith("1.3.6.1.4.1.25506.")){
			   			...
			   			String[] oids =                
							  new String[] {               
								...
								"1.3.6.1.4.1.116.5.11.4.1.1.6.1.3"//dkcHWCSW 内部总线状态
			   			};
								...
								//if(value > 0){
									flag = flag +1;
							   		...
							   		//fanList.add(alist);				   		
							   		  ...
							   		  interfacedata.setCategory("rcable");
...
//							   		 interfacedata.setThevalue("1");
									  SysLogger.info(node.getIpAddress()+" 索引："+index+" 内部总线状态： "+_value);
...
//	    returnHash.put("fan", fanVector);
	    ...
	    if(!(ShareData.getSharedata().containsKey(node.getIpAddress())))
		{
			...
			if(ipAllData == null)ipAllData = new Hashtable();
			if(fanVector != null && fanVector.size()>0)ipAllData.put("rcable",fanVector);
		    ShareData.getSharedata().put(node.getIpAddress(), ipAllData);
		}else
		 {
			if(fanVector != null && fanVector.size()>0)((Hashtable)ShareData.getSharedata().get(node.getIpAddress())).put("rcable",fanVector);
		   
		 }
	    returnHash.put("rcable", fanVector);
	    
	    try{
			AlarmIndicatorsUtil alarmIndicatorsUtil = new AlarmIndicatorsUtil();
			List list = alarmIndicatorsUtil.getAlarmInicatorsThresholdForNode(String.valueOf(node.getId()), AlarmConstant.TYPE_STORAGE, "hds","rcable");
			
			AlarmHelper helper=new AlarmHelper();
			Hashtable<String, EnvConfig> envHashtable=helper.getAlarmConfig(node.getIpAddress(), "rcable");
	    ...
	    //把采集结果生成sql
	    HDSRunCableResultTosql tosql=new HDSRunCableResultTosql();
