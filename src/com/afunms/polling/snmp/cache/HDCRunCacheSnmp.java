...
import com.gatherResulttosql.HDSRunCacheResultTosql;
 ...
 */
public class HDCRunCacheSnmp extends SnmpMonitor {
	//private Hashtable sendeddata = ShareData.getProcsendeddata();
	java.text.SimpleDateFormat sdf = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
	/**
	 * 
	 */
	public HDCRunCacheSnmp() {
	}
		   		  ...
		   		  //if(node.getSysOid().startsWith("1.3.6.1.4.1.2011.") || node.getSysOid().startsWith("1.3.6.1.4.1.25506.")){
			   			...
			   			String[] oids =                
							  new String[] {               
								...
								"1.3.6.1.4.1.116.5.11.4.1.1.6.1.4"//dkcHWCache 缓存状态
			   			};
								...
								//if(value > 0){
									flag = flag +1;
							   		...
							   		//fanList.add(alist);				   		
							   		  ...
							   		  interfacedata.setCategory("rcache");
							   		  interfacedata.setEntity(index);
							   		  interfacedata.setSubentity(num);
							   		  interfacedata.setRestype("dynamic");
							   		  interfacedata.setUnit("");		
							   		  interfacedata.setThevalue(_value);
									  SysLogger.info(node.getIpAddress()+" 索引："+index+" 缓存状态： "+_value);
...
//	    returnHash.put("fan", fanVector);
	    ...
	    if(!(ShareData.getSharedata().containsKey(node.getIpAddress())))
		{
			...
			if(ipAllData == null)ipAllData = new Hashtable();
			if(fanVector != null && fanVector.size()>0)ipAllData.put("rcache",fanVector);
		    ShareData.getSharedata().put(node.getIpAddress(), ipAllData);
		}else
		 {
			if(fanVector != null && fanVector.size()>0)((Hashtable)ShareData.getSharedata().get(node.getIpAddress())).put("rcache",fanVector);
		   
		 }
	    returnHash.put("rcache", fanVector);
	    
	    try{
			AlarmIndicatorsUtil alarmIndicatorsUtil = new AlarmIndicatorsUtil();
			List list = alarmIndicatorsUtil.getAlarmInicatorsThresholdForNode(String.valueOf(node.getId()), AlarmConstant.TYPE_STORAGE, "hds","rcache");
			
			AlarmHelper helper=new AlarmHelper();
			Hashtable<String, EnvConfig> envHashtable=helper.getAlarmConfig(node.getIpAddress(), "rcache");
	    ...
	    //把采集结果生成sql
	    HDSRunCacheResultTosql tosql=new HDSRunCacheResultTosql();
