 ...
 */
public class BDComSnmp extends SnmpMonitor implements MonitorInterface {
...
//				-------------------------------------------------------------------------------------------cpu start
	   		  String temp = snmp.getMibValue(host.getIpAddress(),host.getCommunity(),"1.3.6.1.4.1.3320.9.109.1.1.1.1.5");
	   	  ...
	   	  catch(Exception e)
	   	  {
	   		  e.printStackTrace();
	   		  result = -1;    		  
	   		  SysLogger.error(host.getIpAddress() + "_BDComSnmp 获取CPU信息出错####");
	   	  }	   	  
//			-------------------------------------------------------------------------------------------cpu end	
	   	  ...
	   	  try {
	   		  ...
	   		  if(host.getSysOid().startsWith("1.3.6.1.4.1.9.")){
	   			...
	   			String[] oids =                
					  new String[] {               
						"1.3.6.1.4.1.3320.9.48.1.2.1.2"//bdcomMemoryPoolUtilization5Min
	   			};
	   	  ...
	   	  catch(Exception e)
	   	  {
	   		  e.printStackTrace();
	   		  result = -1;    		  
	   		  SysLogger.error(host.getIpAddress() + "_BDComSnmp",e);
	   	  }	   	  
//			-------------------------------------------------------------------------------------------内存 end	
...
//	   		  }else if(host.getSysOid().startsWith("1.3.6.1.4.1.9.")){
				...
				if(valueArray != null){
						...
						if( value >0){
							...
							SysLogger.info(host.getIpAddress()+" "+cardname+" "+index +" 闪存： "+Integer.parseInt(intvalue+""));
...
//			-------------------------------------------------------------------------------------------system start			
		  try{
			...
			try {
				...
				valueArray = null;
				SysLogger.error(host.getIpAddress() + "_BDComSnmp");
			}
		   	  for(int i=0;i<valueArray.length;i++)
		   	  {
		   		 for(int j=0;j<6;j++){
