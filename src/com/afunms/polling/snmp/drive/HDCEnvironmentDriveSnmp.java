package com.afunms.polling.snmp.drive;
 ...
 */
public class HDCEnvironmentDriveSnmp extends SnmpMonitor {
	//private Hashtable sendeddata = ShareData.getProcsendeddata();
	java.text.SimpleDateFormat sdf = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
	/**
	 * 
	 */
	public HDCEnvironmentDriveSnmp() {
	}
    	...
    	if(ShareData.getTimegatherhash() != null){
    		if(ShareData.getTimegatherhash().containsKey(node.getId()+":equipment")){
    			...
    			if(_result ==1 ){
    				...
    				SysLogger.info("######## "+node.getIpAddress()+" 不在采集驱动时间段内,退出##########");
//    				//清除之前内存中产生的告警信息
//    			    try{
//    			    	//清除之前内存中产生的内存告警信息
								...
								//if(value > 0){
									flag = flag +1;
							   		...
							   		//fanList.add(alist);				   		
							   		  ...
							   		  interfacedata.setCategory("edrive");
							   		  interfacedata.setEntity(index);
							   		  interfacedata.setSubentity(num);
							   		  interfacedata.setRestype("dynamic");
							   		  interfacedata.setUnit("");		
							   		  interfacedata.setThevalue(_value);
									  SysLogger.info(node.getIpAddress()+" 索引："+index+" 驱动状态： "+_value);
...
//	    returnHash.put("fan", fanVector);
	    ...
	    if(!(ShareData.getSharedata().containsKey(node.getIpAddress())))
		{
			...
			if(ipAllData == null)ipAllData = new Hashtable();
			if(fanVector != null && fanVector.size()>0)ipAllData.put("edriver",fanVector);
		    ShareData.getSharedata().put(node.getIpAddress(), ipAllData);
		}else
		 {
			if(fanVector != null && fanVector.size()>0)((Hashtable)ShareData.getSharedata().get(node.getIpAddress())).put("edrive",fanVector);
		   
		 }
	    returnHash.put("edriver", fanVector);
	    
	    try{
			AlarmIndicatorsUtil alarmIndicatorsUtil = new AlarmIndicatorsUtil();
			List list = alarmIndicatorsUtil.getAlarmInicatorsThresholdForNode(String.valueOf(node.getId()), AlarmConstant.TYPE_STORAGE, "hds","edrive");
			
			AlarmHelper helper=new AlarmHelper();
			Hashtable<String, EnvConfig> envHashtable=helper.getAlarmConfig(node.getIpAddress(), "edrive");
	    ...
	    //把采集结果生成sql
	    HDSEnvDriveResultTosql tosql=new HDSEnvDriveResultTosql();
	    tosql.CreateResultTosql(returnHash, node.getIpAddress());
	    SysLogger.info("########完成采集hdc的驱动信息##########");
	    return returnHash;
	}
}
