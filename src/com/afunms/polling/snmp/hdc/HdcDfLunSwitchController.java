...
public class HdcDfLunSwitchController {
	java.text.SimpleDateFormat sdf = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
	/**
	 * 
	 */
	    public HdcDfLunSwitchController() {
	     }
	...
	public Hashtable collect_Data(NodeGatherIndicators alarmIndicatorsNode) {
		SysLogger.info("########开始采集hdc的Lun交换控制器状态信息##########");
			   			...
			   			String[] oids =                
							  new String[] {               
								"1.3.6.1.4.1.116.5.11.4.1.1.6.1.7",//dkcHWBattery
			   					".1.3.6.1.4.1.116.5.11.1.2.5.1.1.4"// dfSwitchControlStatus
			   			};
								...
								//if(value > 0){
									flag = flag +1;
							   		...
							   		//fanList.add(alist);				   		
							   		  ...
							   		  interfacedata.setCategory("rluncon");
							   		  interfacedata.setEntity(index);
//							   		  interfacedata.setEntity("29808");							   		  interfacedata.setSubentity(num);
							   		  interfacedata.setRestype("dynamic");
							   		  interfacedata.setUnit("");		
							   		  interfacedata.setThevalue(_value);
//							   		  interfacedata.setThevalue("0");
									  SysLogger.info(node.getIpAddress()+" 索引："+index+" lun交换控制器状态： "+_value);
	    ...
	    if(!(ShareData.getSharedata().containsKey(node.getIpAddress())))
		{
			...
			if(ipAllData == null)ipAllData = new Hashtable();
			if(fanVector != null && fanVector.size()>0)ipAllData.put("rluncon",fanVector);
		    ShareData.getSharedata().put(node.getIpAddress(), ipAllData);
		}else
		 {
			if(fanVector != null && fanVector.size()>0)((Hashtable)ShareData.getSharedata().get(node.getIpAddress())).put("rluncon",fanVector);
		   
		 }
	    returnHash.put("rluncon", fanVector);
	    
	    try{
			AlarmIndicatorsUtil alarmIndicatorsUtil = new AlarmIndicatorsUtil();
			List list = alarmIndicatorsUtil.getAlarmInicatorsThresholdForNode(String.valueOf(node.getId()), AlarmConstant.TYPE_STORAGE, "hds","rluncon");
			
			AlarmHelper helper=new AlarmHelper();
			Hashtable<String, EnvConfig> envHashtable=helper.getAlarmConfig(node.getIpAddress(), "rluncon");
	...
	if(ipdata.containsKey("rluncon")){
		//风扇
		
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		String allipstr = SysUtil.doip(ip);
	
		Vector fanVector = (Vector) ipdata.get("rluncon");
		if (fanVector != null && fanVector.size() > 0) {
			...
			if (fandata.getRestype().equals("dynamic")) {
				...
				String time = sdf.format(cc);
				String tablename = "rluncon" + allipstr;
				long count = 0;
