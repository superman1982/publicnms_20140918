...
public class HdcDfWwnNumber {

	java.text.SimpleDateFormat sdf = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
	/**
	 * 
	 */
	    public HdcDfWwnNumber() {
	     }
	...
	public Hashtable collect_Data(NodeGatherIndicators alarmIndicatorsNode) {
		SysLogger.info("########开始采集hdc的编号使用状态信息##########");
								...
								//if(value > 0){
									flag = flag +1;
							   		...
							   		//fanList.add(alist);				   		
							   		  ...
							   		  interfacedata.setCategory("rnumber");
...
//							   		  interfacedata.setThevalue("1");
									  SysLogger.info(node.getIpAddress()+" 索引："+index+" 编号使用状态： "+_value);
	    ...
	    if(!(ShareData.getSharedata().containsKey(node.getIpAddress())))
		{
			...
			if(ipAllData == null)ipAllData = new Hashtable();
			if(fanVector != null && fanVector.size()>0)ipAllData.put("rnumber",fanVector);
		    ShareData.getSharedata().put(node.getIpAddress(), ipAllData);
		}else
		 {
			if(fanVector != null && fanVector.size()>0)((Hashtable)ShareData.getSharedata().get(node.getIpAddress())).put("rnumber",fanVector);
		   
		 }
	    returnHash.put("rnumber", fanVector);
	    
	    try{
			AlarmIndicatorsUtil alarmIndicatorsUtil = new AlarmIndicatorsUtil();
			List list = alarmIndicatorsUtil.getAlarmInicatorsThresholdForNode(String.valueOf(node.getId()), AlarmConstant.TYPE_STORAGE, "hds","rnumber");
			
			AlarmHelper helper=new AlarmHelper();
			Hashtable<String, EnvConfig> envHashtable=helper.getAlarmConfig(node.getIpAddress(), "rnumber");
	...
	if(ipdata.containsKey("rnumber")){
		//风扇
		
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		String allipstr = SysUtil.doip(ip);
	
		Vector fanVector = (Vector) ipdata.get("rnumber");
		if (fanVector != null && fanVector.size() > 0) {
			...
			if (fandata.getRestype().equals("dynamic")) {
				...
				String time = sdf.format(cc);
				String tablename = "rnumber" + allipstr;
				long count = 0;
