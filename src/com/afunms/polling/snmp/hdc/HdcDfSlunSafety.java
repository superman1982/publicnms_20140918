...
public class HdcDfSlunSafety {
	...
	public Hashtable collect_Data(NodeGatherIndicators alarmIndicatorsNode) {
		SysLogger.info("########开始采集hdc的wwn safety状态信息##########");
...
//							   		  interfacedata.setThevalue("0");
									  SysLogger.info(node.getIpAddress()+" 索引："+index+" wwn safety： "+_value);
	    ...
	    if(!(ShareData.getSharedata().containsKey(node.getIpAddress())))
		{
			...
			if(ipAllData == null)ipAllData = new Hashtable();
			if(fanVector != null && fanVector.size()>0)ipAllData.put("rsafety",fanVector);
		    ShareData.getSharedata().put(node.getIpAddress(), ipAllData);
		}else
		 {
			if(fanVector != null && fanVector.size()>0)((Hashtable)ShareData.getSharedata().get(node.getIpAddress())).put("rsafety",fanVector);
