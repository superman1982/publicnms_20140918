...
public class LoadScoOpenServerFile {
	private String ipaddress;

	public LoadScoOpenServerFile(String ipaddress) {
		this.ipaddress = ipaddress;
	}

	public LoadScoOpenServerFile() {
	...
	public Hashtable collect_Data(NodeGatherIndicators alarmIndicatorsNode) {
		...
		//SysLogger.info("versionContent===="+versionContent);
		String[] versionLineArr = versionContent.split("\n");
		if (versionLineArr != null && versionLineArr.length > 0) {
			if (versionLineArr[1] != null) {
				nodeconfig.setHostname(versionLineArr[1]);
			}
		...
		//SysLogger.info("swap_Content===="+swap_Content);
		...
		for (int i = 0; i < diskLineArr.length; i++) {
			...
			if ((tmpData != null) && (tmpData.length == 6)) {
				...
				diskdata.setUnit("%");
				try {
					diskdata.setThevalue(Float.toString(Float.parseFloat(tmpData[5].substring(0, tmpData[5].indexOf("%")))));
				} catch (Exception ex) {
					continue;
				}
				...
				try {
					...
					if (disk_v != null && disk_v.size() > 0) {
						...
						pastutil = Float.parseFloat(tmpData[5].substring(0, tmpData[5].indexOf("%")));
					}
					if (pastutil == 0) {
						pastutil = Float.parseFloat(tmpData[5].substring(0, tmpData[5].indexOf("%")));
					}
					if (Float.parseFloat(tmpData[5].substring(0, tmpData[5].indexOf("%"))) - pastutil > 0) {
						diskinc = (Float.parseFloat(tmpData[5].substring(0, tmpData[5].indexOf("%"))) - pastutil) + "";
					}
					diskdata = new Diskcollectdata();
				...
				allblocksize = Float.parseFloat(tmpData[2]);
				float allsize = 0.0f;
				...
				float FreeintSize = 0;
				FreeintSize = Float.parseFloat(tmpData[4]);
				...
				diskflag = diskflag + 1;
			} else if ((tmpData != null) && (!"Mount".equalsIgnoreCase(tmpData[0]))) {
				diskLabel = tmpData[0];
				...
				diskdata.setUnit("%");
				try {
					diskdata.setThevalue(Float.toString(Float.parseFloat(tmpData[5]) * 100 / Float.parseFloat(tmpData[3])));
				} catch (Exception ex) {
					continue;
				}
				diskVector.addElement(diskdata);
				...
				try {
					...
					if (disk_v != null && disk_v.size() > 0) {
						...
						pastutil = Float.parseFloat(tmpData[5]) * 100 / Float.parseFloat(tmpData[3]);
					}
					if (pastutil == 0) {
						pastutil = Float.parseFloat(tmpData[5]) * 100 / Float.parseFloat(tmpData[3]);
					}
					if (Float.parseFloat(tmpData[5]) * 100 / Float.parseFloat(tmpData[3]) - pastutil > 0) {
						diskinc = (Float.parseFloat(tmpData[5]) * 100 / Float.parseFloat(tmpData[3]) - pastutil) + "";
					}
					diskdata = new Diskcollectdata();
					diskdata.setIpaddress(host.getIpAddress());
					diskdata.setCollecttime(date);
				...
				float allblocksize = 0;
				allblocksize = Float.parseFloat(tmpData[3]);
				float allsize = 0.0f;
				allsize = allblocksize;
				allsize = allsize / 1024;
				diskdata.setUnit("M");
				...
				float FreeintSize = 0;
				FreeintSize = Float.parseFloat(tmpData[5]);
		...
		if (PhysicalMemCap > 0) {
			...
			memorydata.setRestype("static");
			memorydata.setUnit("M");
			memorydata.setThevalue(Float.toString(PhysicalMemCap - freePhysicalMemory));
			memoryVector.addElement(memorydata);
			// 内存使用率
			memorydata = new Memorycollectdata();
			memorydata.setIpaddress(ipaddress);
		...
		if (SwapMemCap > 0) {
			...
			// System.out.println("交换分区使用率
			// "+Math.round(usedSwapMemory*100/SwapMemCap)+"");
			memoryVector.addElement(memorydata);
		}
		...
		for (int i = 1; i < process_LineArr.length; i++) {
			...
			if ((processtmpData != null) && (processtmpData.length >= 8)) {
				...
				if ("UID".equalsIgnoreCase(USER))
					continue;
				String cmd = processtmpData[7];
				String vbstring8 = processtmpData[4];// STIME
				String vbstring5 = processtmpData[6];// cputime
				String vbstring2 = "应用程序";
				String vbstring3 = "";
				String vbstring4 = "0";
				String vbstring6 = "0.0";// %mem
				String vbstring7 = "0";// %CPU
