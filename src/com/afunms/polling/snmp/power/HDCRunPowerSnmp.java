...
//					   		interfacedata.setThevalue("3");
					   		  SysLogger.info(node.getIpAddress()+" 索引 ："+index+" 电源状态： "+_value);
...
//				-------------------------------------------------------------------------------------------电源 end
			...
			if(!(ShareData.getSharedata().containsKey(node.getIpAddress())))
			{
				...
				if(ipAllData == null)ipAllData = new Hashtable();
				if(powerVector != null && powerVector.size()>0)ipAllData.put("rpower",powerVector);
			    ShareData.getSharedata().put(node.getIpAddress(), ipAllData);
			}else
			 {
				if(powerVector != null && powerVector.size()>0)((Hashtable)ShareData.getSharedata().get(node.getIpAddress())).put("rpower",powerVector);
			 }
	    ...
	    powerVector=null;
	    //把采集结果生成sql
//	    SysLogger.info("########看看走没走创建SQL语句的方法##########");
