...
//    			    }
		...
		try {
			...
			//-------------------------------------------------------------------------------------------电源 start
		   	  try {
		   		  String temp = "0";
		   		  if(node.getSysOid().startsWith("1.3.6.1.4.1.2011.")|| node.getSysOid().equalsIgnoreCase("1.3.6.1.4.1.25506.1.149")){
		   		   ...
		   		   //wxy add
		   			if(node.getSysOid().startsWith("1.3.6.1.4.1.25506.1.515")||node.getSysOid().startsWith("1.3.6.1.4.1.25506.1.354")){
		   				oids =                
							  new String[] {               
								"1.3.6.1.4.1.25506.8.35.9.1.1.1.1",//hwDevMPowerNum
								"1.3.6.1.4.1.25506.8.35.9.1.1.1.2"//hwDevMPowerStatus
			   			};
		   			}
		   			//valueArray = SnmpUtils.getTemperatureTableData(node.getIpAddress(), node.getCommunity(), oids, node.getSnmpversion(), 2, 1000);
		   			valueArray = SnmpUtils.getTemperatureTableData(node.getIpAddress(),node.getCommunity(),oids,node.getSnmpversion(),
...
//				-------------------------------------------------------------------------------------------电源 end
	    ...
	    try{
			...
			List list = alarmIndicatorsUtil.getAlarmInicatorsThresholdForNode(String.valueOf(node.getId()), AlarmConstant.TYPE_NET, "h3c","power");
