...
import com.gatherResulttosql.HDSEnvironmentPowerResultTosql;
 ...
 */
public class HDCEnvironmentPowerSnmp extends SnmpMonitor {
	private Hashtable sendeddata = ShareData.getProcsendeddata();
	java.text.SimpleDateFormat sdf = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
	/**
	 * 
	 */
	public HDCEnvironmentPowerSnmp() {
	}
    	...
    	if(ShareData.getTimegatherhash() != null){
    		if(ShareData.getTimegatherhash().containsKey(node.getId()+":equipment")){
    			...
    			if(_result ==1 ){
    				...
    				SysLogger.info("######## "+node.getIpAddress()+" 不在采集power时间段内,退出##########");
//    				//清除之前内存中产生的告警信息
//    			    try{
//    			    	//清除之前内存中产生的内存告警信息
		   		  ...
		   		  //if(node.getSysOid().startsWith("1.3.6.1.4.1.25506.")){
		   			...
		   			String[] oids =                
						  new String[] {               
							...
							"1.3.6.1.4.1.116.5.11.4.1.1.7.1.2"//dkuHWPS
		   			};
					...
					if(valueArray != null){
							...
							flag = flag +1;
					   		...
					   		//powerList.add(alist);				   		
					   		  ...
					   		  interfacedata.setCategory("epower");
...
//				-------------------------------------------------------------------------------------------电源 end
			...
			if(!(ShareData.getSharedata().containsKey(node.getIpAddress())))
			{
				...
				if(ipAllData == null)ipAllData = new Hashtable();
				if(powerVector != null && powerVector.size()>0)ipAllData.put("epower",powerVector);
			    ShareData.getSharedata().put(node.getIpAddress(), ipAllData);
			}else
			 {
				if(powerVector != null && powerVector.size()>0)((Hashtable)ShareData.getSharedata().get(node.getIpAddress())).put("epower",powerVector);
			 }
			
	    returnHash.put("epower", powerVector);
	    try{
			AlarmIndicatorsUtil alarmIndicatorsUtil = new AlarmIndicatorsUtil();
			List list = alarmIndicatorsUtil.getAlarmInicatorsThresholdForNode(String.valueOf(node.getId()), AlarmConstant.TYPE_STORAGE, "hds","epower");
			AlarmHelper helper=new AlarmHelper();
			Hashtable<String, EnvConfig> envHashtable=helper.getAlarmConfig(node.getIpAddress(), "epower");
	    ...
	    //把采集结果生成sql
	    HDSEnvironmentPowerResultTosql tosql=new HDSEnvironmentPowerResultTosql();
