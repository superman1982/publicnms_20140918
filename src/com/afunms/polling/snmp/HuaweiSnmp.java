...
//				-------------------------------------------------------------------------------------------cpu start
	   		  String temp = "0";
	   		  if(host.getSysOid().startsWith("1.3.6.1.4.1.2011.") || host.getSysOid().startsWith("1.3.6.1.4.1.45.")){
	   			//temp = snmp.getMibValue(host.getIpAddress(),host.getCommunity(),"1.3.6.1.4.1.2011.6.1.1.1.4.0");
	   			String[][] valueArray = null;
	   			String[] oids =                
					  new String[] {               
						"1.3.6.1.4.1.2011.6.1.1.1.4.0"};
	   			String[] oids2 = new String[] {"1.3.6.1.4.1.2011.10.2.6.1.1.1.1.6.0"};
					...
					if(valueArray != null && valueArray.length > 0){
					   	  for(int i=0;i<valueArray.length;i++)
					   	  {
								...
								if( value >0){
									int intvalue = Math.round(value); 
									allvalue = allvalue +intvalue;
									SysLogger.info(host.getIpAddress()+" 闪存： "+Integer.parseInt(intvalue+"")+" 总闪存:"+allvalue);
									flag = flag +1;
							   		List alist = new ArrayList();
							   		alist.add("");
							   		alist.add(usedperc);
...
//			-------------------------------------------------------------------------------------------缓存 start
	   	  try {
	   		  ...
	   		  if(host.getSysOid().startsWith("1.3.6.1.4.1.2011.") || host.getSysOid().startsWith("1.3.6.1.4.1.25506.")){
		   			...
		   			String[] oids =                
						  new String[] {  
							"1.3.6.1.4.1.2011.6.1.2.2.1.3",//hwBufCurrentTotal
							"1.3.6.1.4.1.2011.6.1.2.2.1.4"//hwBufCurrentUsed
		   			};
		   			valueArray = snmp.getTemperatureTableData(host.getIpAddress(),host.getCommunity(),oids);
		   			int allvalue=0;
					...
					if(valueArray != null && valueArray.length > 0){
					   	  ...
					   	  {
						   		String sizevalue = valueArray[i][0];
						   		String usedvalue = valueArray[i][1];
						   		String index = valueArray[i][2];
						   		float value=0.0f;
						   		String usedperc = "0";
						   		if(Long.parseLong(sizevalue) > 0)
						   			value = (Long.parseLong(usedvalue))*100/(Long.parseLong(sizevalue));
						   		
								if( value >0){
									int intvalue = Math.round(value); 
									allvalue = allvalue +intvalue;
									//SysLogger.info(host.getIpAddress()+" 索引："+index+" 缓存 总大小："+sizevalue+" 已使用："+usedvalue+" 利用率："+Integer.parseInt(intvalue+""));
									flag = flag +1;
							   		List alist = new ArrayList();
							   		alist.add(index);
							   		alist.add(usedperc);
									...
									SysLogger.info(host.getIpAddress()+" 索引："+index+" 缓存： "+Integer.parseInt(intvalue+""));
...
//			-------------------------------------------------------------------------------------------缓存 end	
	   	  ...
	   	  try {
	   		  String temp = "0";
	   		  if(host.getSysOid().startsWith("1.3.6.1.4.1.2011.")){
	   		  }else if(host.getSysOid().startsWith("1.3.6.1.4.1.25506.") || host.getSysOid().startsWith("1.3.6.1.4.1.2011.")){
	   			String[][] valueArray = null;
	   			String[] oids =                
					  new String[] {       
	   	  ...
	   	  try {
	   		  String temp = "0";
	   		  if(host.getSysOid().startsWith("1.3.6.1.4.1.25506.") || host.getSysOid().startsWith("1.3.6.1.4.1.2011.")){
	   			String[][] valueArray = null;
	   			String[] oids =                
					  new String[] {       
	   					//"1.3.6.1.4.1.43.45.1.10.2.6.1.1.1.1.12"
						...
						if(value >0){
							flag = flag +1;
					   		...
					   		voltageList.add(alist);				   		
					   		  ...
					   		  interfacedata.setThevalue(result+"");
							  SysLogger.info(host.getIpAddress()+" 电压： "+result);
					...
					if(valueArray != null){
							...
							flag = flag +1;
					   		...
					   		powerList.add(alist);				   		
					   		  ...
					   		  interfacedata.setUnit("");		
					   		  interfacedata.setThevalue(_value);
							  SysLogger.info(host.getIpAddress()+" 索引："+index+" 电源状态： "+_value);
					...
					if(valueArray != null){
							...
							flag = flag +1;
					   		...
					   		powerList.add(alist);				   		
					   		  ...
					   		  interfacedata.setUnit("");		
					   		  interfacedata.setThevalue(_value);
							  SysLogger.info(host.getIpAddress()+" 软件版本： "+_value);
							  //powerVector.addElement(interfacedata);		   		
					   	  }
					}	   			     			  
	   		  } 
...
//										System.out.println("流速信息=========="+allInOctetsSpeed+"---"+allOutOctetsSpeed+"---"+allInOctetsSpeed+"---"+allOctetsSpeed);
...
//				System.out.println("@@@@入口流速为："+Long.toString(allInOctetsSpeed*8));
...
//				System.out.println("@@@@出口流速为："+Long.toString(allOutOctetsSpeed*8));
...
//				System.out.println("@@@@综合流速为："+Long.toString(allOctetsSpeed));
