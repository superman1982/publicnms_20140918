 ...
 */
public class HDCEnvironmentFanSnmp extends SnmpMonitor {
	//private Hashtable sendeddata = ShareData.getProcsendeddata();
	java.text.SimpleDateFormat sdf = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
	/**
	 * 
	 */
	public HDCEnvironmentFanSnmp() {
	}
	...
	public Hashtable collect_Data(NodeGatherIndicators alarmIndicatorsNode) {
		SysLogger.info("########开始采集hdc的风扇信息##########");
		Hashtable returnHash=new Hashtable();
		Vector fanVector=new Vector();
		Host node = (Host)PollingEngine.getInstance().getNodeByID(Integer.parseInt(alarmIndicatorsNode.getNodeid()));
//		System.out.println("####node############"+node+"#########id#######"+alarmIndicatorsNode.getNodeid());
								...
								//if(value > 0){
									flag = flag +1;
							   		...
							   		//fanList.add(alist);	
							   		  ...
							   		  interfacedata.setCategory("efan");
							   		  ...
							   		  //测试----
//							   		System.out.println(index+"***************2222222222****************"+_value);
//							   		  interfacedata = new Interfacecollectdata();
//							   		  interfacedata.setIpaddress(node.getIpAddress());
//							   		  interfacedata.setCollecttime(date);
//							   		  interfacedata.setCategory("efan");
//							   		  interfacedata.setEntity("111");
//							   		  interfacedata.setSubentity("2222");
//							   		  interfacedata.setRestype("dynamic");
//							   		  interfacedata.setUnit("");		
//							   		  interfacedata.setThevalue("abcdefg,,,,");
							   		  
									  SysLogger.info(node.getIpAddress()+" 索引："+index+" 风扇状态： "+_value);
...
//	    returnHash.put("fan", fanVector);
	    ...
	    if(!(ShareData.getSharedata().containsKey(node.getIpAddress())))
		{
			...
			if(ipAllData == null)ipAllData = new Hashtable();
			if(fanVector != null && fanVector.size()>0)ipAllData.put("efan",fanVector);
		    ShareData.getSharedata().put(node.getIpAddress(), ipAllData);
		}else
		 {
			if(fanVector != null && fanVector.size()>0)((Hashtable)ShareData.getSharedata().get(node.getIpAddress())).put("efan",fanVector);
		   
		 }
	    returnHash.put("efan", fanVector);
//	    Hashtable data1 = (Hashtable) ShareData.getSharedata().get(node.getIpAddress());
//	    Vector v = new Vector();
//	    v = (Vector) data1.get("efan");
//	    System.out.println("::::::::::::::::::::::::::::::::::::"+v.size()+v.get(0));
	    try{
			AlarmIndicatorsUtil alarmIndicatorsUtil = new AlarmIndicatorsUtil();
			List list = alarmIndicatorsUtil.getAlarmInicatorsThresholdForNode(String.valueOf(node.getId()), AlarmConstant.TYPE_STORAGE, "hds","efan");
			
			AlarmHelper helper=new AlarmHelper();
			Hashtable<String, EnvConfig> envHashtable=helper.getAlarmConfig(node.getIpAddress(), "efan");
	    ...
	    //把采集结果生成sql
	    HDSEnvfanResultTosql tosql=new HDSEnvfanResultTosql();
