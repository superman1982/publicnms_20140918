 ...
 */
public class CommonCpuSnmp extends SnmpMonitor {
	java.text.SimpleDateFormat sdf = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
	/**
	 * 
	 */
	public CommonCpuSnmp() {
	}
	...
	public Hashtable collect_Data(NodeGatherIndicators nodeGatherIndicators) {	
		...
		if(!node.isManaged())return returnHash;
		...
		try {
			System.out.println("Start collect data as ip "+node.getIpAddress());
		    	...
		    	if(cpuVector != null && cpuVector.size() > 0){
					for(int i = 0 ; i < cpuVector.size(); i++){
						...
						if("Utilization".equals(cpucollectdata.getEntity())){
							CheckEventUtil checkutil = new CheckEventUtil();
							checkutil.hardwareInfo(node, "cpu", cpuList.size()+"");
...
//			List list = alarmIndicatorsUtil.getAlarmInicatorsThresholdForNode(String.valueOf(node.getId()), AlarmConstant.TYPE_HOST, "windows","cpu");
