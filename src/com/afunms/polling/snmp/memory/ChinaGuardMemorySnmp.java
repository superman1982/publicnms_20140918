...
public class ChinaGuardMemorySnmp extends SnmpMonitor {
	...
	public Hashtable collect_Data(NodeGatherIndicators nodeGatherIndicators) {	
			  ...
			  try {
				  //-------------------------------------------------------------------------------------------内存 start
		   		  ...
		   		  if(node.getSysOid().startsWith("1.3.6.1.4.1.28880.")){
		   			...
		   			String[] oids =                
						  new String[] {               
							"1.3.6.1.4.1.28880.2.10.2.2",//MemoryPoolUsed
		...
		try {
			...
			e.printStackTrace();
			SysLogger.error("westone 内存 告警出错" , e);
		}
	    memoryVector=null;
//		//对内存值进行告警检测
//	    try{
//			AlarmIndicatorsUtil alarmIndicatorsUtil = new AlarmIndicatorsUtil();
//			List list = alarmIndicatorsUtil.getAlarmInicatorsThresholdForNode(String.valueOf(node.getId()), AlarmConstant.TYPE_NET, "cisco","memory");
