 ...
 * 
 */
public class HeapSnmp extends SnmpMonitor {
	...
	public Hashtable collect_Data(NodeGatherIndicators nodeGatherIndicators) {
		...
		try {
			...
			try {
				...
				heapValue = weblogicsnmp.collectHeapData();
			} catch (Exception e) {
				e.printStackTrace();
			}
			if(heapValue!=null){
				SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
				returndata.put("heapValue", heapValue);
				if (!(ShareData.getWeblogicdata().containsKey(weblogicconf.getIpAddress()))) {
					...
					hash.put("heapValue", returndata.get("heapValue"));
				}
				...
				String nodeid = id;
				List heapValuesList = (ArrayList)heapValue;
